...
def get_content():
    ...
    from Deep_Learning.visuals.mlp_xor_component import MLP_XOR_HTML, MLP_XOR_HEIGHT

    # Build an absolute path to the PNG so it works regardless of the
    # working directory Streamlit is launched from.
    png_path = Path(__file__).parent.parent / "visuals" / "RAG_image.png"
    mlp_img = _image_to_html(
        str(png_path),
        alt="RAG Architecture",
        width="80%"
    )
    theory_with_images = THEORY.replace("{{RAG_IMAGE}}", mlp_img)
...
"""
    ...
    Legend:  Q = Question   [*] = LLM/Brain   [DB] = Relational DB   [<>] = Graph DB   {} = Vector/JSON store
           [=] = Document   [V] = Filter   (o) = Router node   --> = flow   ~~> = ranked   . . = section border
    ...
    |  .  Q-->[*]-->[DB]  Q-->[*]-->[<>] Q-->[*]-->[{}]  .      .    /                      |                          .  |
    |  .                                                 .      .  [=]                    (CRAG)                       .  |
    |  .  Text-to-SQL     Text-to-Cypher  Self-query     .      .  [=]                                                 .  |
    |  .  NL to SQL/      NL to Cypher    Auto-meta      .      .  Re-Rank, RankGPT, RAG-Fusion                        .  |
    |  .  PGVector        query lang      filters        .      .  Rank/filter/compress docs by relevance              .  |
    |  . . . . . . . . . . . . . . . . . . . . . . . . . .      .  - - - - - - - - - - - - - - - - - - - - - - - - - - .  |
    |                                                           .  Active retrieval                        CRAG        .  |
    +- - - - - - - - - - - - - - - - - - - - - - - - - - +         .  {}--------->[=]  Re-retrieve / retrieve          .  |
    |  Query Translation                                 |         .             from new sources if docs not relevant .  |
    |  . . . . . . . . . . . . . . . . . . . . .         |         . . . . . . . . . . . . . . . . . . . . . . . . . . .  |
    |  .  Query Decomposition | Pseudo-docs    .         |                              |                                 |
    |  .                      |                .         |                              v                                 |
    |  .  Q-->[*]-->[Sub/Step-back Q(s)]       .         |                       .----------.                             |
    |  .           |          |                .         |                       | Graph DB |                             |
    |  .  Multi-query,Step-back,RAG-Fusion     .         |                       '----------'                             |
    |  .  Decompose/rephrase input question    .         |                              |                                 |
    |  .  - - - - - - - - - - - - - - - - -    .         |          Q --> (o)-->(o)--->(o)-->  .----------.               |
    |  .  Pseudo-documents | HyDE              .         |                              |      |Relational|-->[Docs]->[V]-->[*]-->Answer
    |  .  Q-->[*]-->[docs] | Hypothetical      .         |                              |      |    DB    |              ^       |
    |  . . . . . . . . . . . . . . . . . . . . .         |                              v      '----------'              |       |
    |                                                    |                       .-----------.                            |       |
    +- - - - - - - - - - - - - - - - - - - - - - - - - - +                       | Vectorstore|<---------------------------+       |
    |  Routing                                           |                       '-----------'                            |       |
    ...
    |  .   \              |  \                 .         |                       .               ^                   .    |       |
    |  .   [<>]           |  Embed             .         |                       .               |                   .    |       |
    |  .                  |  [*]-->[Prompt#2]  .         |                       .  Self-RAG, RRR                    .    |       |
    |  .  Let LLM choose  | Embed Q & choose   .         |                       .  Use generation quality to inform .    |       |
    |  .  DB on question  | prompt by sim      .         |                       .  question re-writing / re-retrieval.   |       |
    |  . . . . . . . . . . . . . . . . . . . . .         |                       . . . . . . . . . . . . . . . . . .     |       |
    +- - - - - - - - - - - - - - - - - - - - - - - - - - +                                                                        |
                                                                                                                         |
    +====================================================================================================================+
    |  Indexing  (feeds Vectorstore above via dashed line)                                                              |
    |  . . . . . . . . . . . . .  . . . . . . . . . . . .  . . . . . . . . . . . . .  . . . . . . . . . . . . .  .   |
    |  .  Chunk Optimization   .  .  Multi-rep indexing  .  .  Specialized Embeddings.  .  Hierarchical Indexing  .  . |
    |  .                       .  .                      .  .                        .  .                         .  . |
    |  .  [doc]                .  .  [doc]               .  .  [doc]                 .  .         Summaries       .  . |
    |  .   |  Split by:        .  .   |                  .  .   |                    .  .  Splits  .--> [Cluster] .  . |
    |  .   +-> Characters      .  .  [*]                 .  .  [*]                   .  .   |    [=]      ...    .  . |
    |  .   +-> Sections        .  .   |                  .  .   |                    .  .   |      '--> [Cluster] .  . |
    |  .   +-> Semantic        .  .  Summary --> {}       .  .  [0.1, 0.3, ...]      .  .  [*]                   .  . |
    |  .   +-> Delimiters      .  .                      .  .                        .  .                         .  . |
    |  .                       .  .  Parent Document     .  .  Fine-tuning, ColBERT  .  .  RAPTOR                 .  . |
    |  .  Semantic Splitter     .  .  Dense X            .  .  Domain-specific /     .  .  Tree of doc summary    .  . |
    |  .  Optimize chunk size   .  .  Compact retrieval  .  .  advanced embeddings   .  .  at various abstraction  .  . |
    |  .  used for embedding    .  .  units (e.g. summary).  .                        .  .  levels                .  . |
    |  . . . . . . . . . . . . .  . . . . . . . . . . . .  . . . . . . . . . . . . .  . . . . . . . . . . . . .  .   |
    +====================================================================================================================+
"""