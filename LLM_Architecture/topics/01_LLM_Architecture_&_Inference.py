...
The size of K, Q, V matrix is determined by architecture hyperparameters decided 
    ...
    ╚══════════════════════════════════════════════════════════════════╝
      ...
      W_Q  shape  =  (d_model  ×  d_head × num_heads)
      W_K  shape  =  (d_model  ×  d_head × num_heads)
      W_V  shape  =  (d_model  ×  d_head × num_heads)
    
      Where:
      ┌─────────────────┬────────────────────────────────────────┐
      │  d_model        │  width of every token's vector         │
      │                 │  (the "size of the model")             │
      ├─────────────────┼────────────────────────────────────────┤
      │  d_head         │  dimension of each attention head      │
      │                 │  usually = d_model ÷ num_heads         │
      ├─────────────────┼────────────────────────────────────────┤
      │  num_heads      │  how many attention heads per layer    │
      └─────────────────┴────────────────────────────────────────┘
    
      Most architectures set:
    
      d_head  =  d_model ÷ num_heads
    
      So the total projection size  =  d_model × d_model
      Which means W_Q, W_K, W_V are all SQUARE matrices
      when you count all heads together.
    ...
    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHY SQUARE?  — THE INTUITION                                    ║
    ╚══════════════════════════════════════════════════════════════════╝
    
      Input token vector has d_model dimensions.
      Output of attention must also have d_model dimensions
      so it can feed into the next layer cleanly.
      ...
      Input shape:   (1  ×  d_model)
                      ...
                      (d_model × d_model)
                            ↓
      Output shape:  (1  ×  d_model)   ← same size, different meaning
    ...
    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHAT DETERMINES THE SIZE — DECISION TREE                        ║
    ╚══════════════════════════════════════════════════════════════════╝
    
      Before training begins, engineers decide:
    
      "How big should the model be?"
               │
               ├──▶  choose d_model  (e.g. 4096)
               │              │
               │              └──▶  W_Q, W_K, W_V
               │                    automatically = 4096 × 4096
               │
               ├──▶  choose num_heads  (e.g. 32)
               │              │
               │              └──▶  d_head = 4096 ÷ 32 = 128
               │
               └──▶  choose num_layers  (e.g. 32)
                              │
                              └──▶  you get 32 SEPARATE sets of
                                    W_Q, W_K, W_V
                                    (each layer has its own weights)
    
    
      Training dataset size does NOT change the shape.
      ─────────────────────────────────────────────────
      More data → better VALUES inside the matrix
      More data → does NOT change the SHAPE of the matrix
    
      It is like asking:
      "does studying more change the size of your brain?"
      No. It changes what is stored inside it.
    
    
    
    ╔══════════════════════════════════════════════════════════════════╗
    ║  HOW MANY W_Q W_K W_V MATRICES EXIST IN TOTAL?                   ║
    ╚══════════════════════════════════════════════════════════════════╝
    
      Each layer has its OWN independent W_Q, W_K, W_V.
      They are not shared across layers.
    
      LLaMA 3 8B example:
    
      num_layers  =  32
      W_Q per layer  =  4096 × 4096  =  16,777,216 numbers
      W_K per layer  =  4096 × 4096  =  16,777,216 numbers
      W_V per layer  =  4096 × 4096  =  16,777,216 numbers
    
      Total just for QKV weights:
    
      3  ×  16,777,216  ×  32 layers
      =  1,610,612,736  numbers
      ≈  1.6 BILLION parameters
         just for QKV alone
    
      (The full 8B model has ~8 billion total parameters
       including FFN, embeddings, and output projection)
    
      ┌──────────────────────────────────────────────────┐
      │  Layer 1   W_Q₁  W_K₁  W_V₁  ← unique weights    │
      │  Layer 2   W_Q₂  W_K₂  W_V₂  ← unique weights    │
      │  Layer 3   W_Q₃  W_K₃  W_V₃  ← unique weights    │
      │  ...                                             │
      │  Layer 32  W_Q₃₂ W_K₃₂ W_V₃₂ ← unique weights    │
      │                                                  │
      │  Each layer learned DIFFERENT projections.       │
      │  Early layers learned syntactic projections.     │
      │  Later layers learned semantic projections.      │
      └──────────────────────────────────────────────────┘
    
So if additional heads are added in fine tuning does these weights gets modified or a new copy of these weights is
formed after re training on the dataset
    
This is a really important question because the answer reveals how fine-tuning actually works.
The short answer is — you cannot add heads during fine-tuning in the traditional sense. Here is why:
    ...
    ╚══════════════════════════════════════════════════════════════════╝
      ...
      The number of attention heads is BAKED INTO the architecture.
      It is not a value you can change after training without
      fundamentally changing the shape of the weight matrices.
    
      num_heads changes  →  d_head changes
      d_head changes     →  W_Q, W_K, W_V shape changes
      shape changes      →  completely different model architecture
    
      You cannot load old weights into a new shape.
      They are incompatible like trying to plug a
      3-pin connector into a 2-pin socket.
    
      ┌─────────────────────────────────────────────────┐
      │  Original model:  4 heads, d_head=128           │
      │  W_Q shape:  512 × 512                          │
      │                                                 │
      │  "Add 2 more heads":  6 heads, d_head=128       │
      │  W_Q shape needs to be: 512 × 768               │
      │                           ↑                     │
      │                    completely different shape   │
      │                    old weights cannot fit in    │
      └─────────────────────────────────────────────────┘
    
    ╔══════════════════════════════════════════════════════════════════╗
    ║  SO WHAT ACTUALLY HAPPENS IN FINE-TUNING?                        ║
    ║  THREE DIFFERENT SCENARIOS                                       ║
    ╚══════════════════════════════════════════════════════════════════╝
    
      SCENARIO 1 — FULL FINE-TUNING  (most straightforward)
      ─────────────────────────────────────────────────────
    
      Same architecture. Same shapes. Values updated.
    
      Before fine-tuning:
      ┌──────────────────────────────────────────────┐
      │  W_Q  (4096 × 4096)                          │
      │  [0.234  0.891  -0.123  0.445  ...]          │
      │  [0.667 -0.234   0.789  0.123  ...]          │
      │  [...                            ]           │
      └──────────────────────────────────────────────┘
    
      Fine-tune on medical data for 10,000 steps...
      Backpropagation nudges every value slightly.
    
      After fine-tuning:
      ┌──────────────────────────────────────────────┐
      │  W_Q  (4096 × 4096)   ← SAME SHAPE           │
      │  [0.241  0.887  -0.119  0.451  ...]          │
      │  [0.659 -0.238   0.793  0.118  ...]          │
      │  [...                            ]           │
      └──────────────────────────────────────────────┘
    
      Shape:   identical ✓
      Values:  slightly different ✓
      Heads:   still 32, nothing added ✓
    
      Result: ONE model file with updated weights.
              No copy. The original values are GONE
              unless you saved a checkpoint.
    
    
    SCENARIO 2 — LoRA FINE-TUNING  (industry standard today)
      ─────────────────────────────────────────────────────────
      Low-Rank Adaptation — this is the closest thing to
      "adding" capacity without touching original weights.
    
      Original W_Q is FROZEN (values locked, never change).
      Two tiny NEW matrices A and B are added alongside it.
    
      During inference:
    
      Original path:
      ┌────────────────────────────────────────────────────┐
      │  W_Q  (4096 × 4096)   ← FROZEN, never updated      │
      │  [0.234  0.891  -0.123  ...]                       │
      │  exact same values as base model                   │
      └──────────────────────┬─────────────────────────────┘
                             │
      LoRA adapter path:     │
      ┌──────────────────┐   │
      │  A  (4096 × 16)  │   │   ← tiny matrix, rank=16
      │  random init     │   │
      └────────┬─────────┘   │
               │             │
      ┌────────▼─────────┐   │
      │  B  (16 × 4096)  │   │   ← tiny matrix, rank=16
      │  zero init       │   │     (starts at zero so
      └────────┬─────────┘   │      adapter starts neutral)
               │             │
               └──────┬──────┘
                      │ ADD together
                      ▼
      ...
      │                                                 │
      │  Full W_Q:      4096 × 4096  = 16.7M params     │
      │  LoRA A + B:    4096×16 + 16×4096 = 131K params │
      │                                    ↑            │
      │                               99.2% fewer       │
      │                               parameters to     │
      │                               train             │
      ...
      After fine-tuning you have TWO files:
      base_model.safetensors   (unchanged, 14GB)
      lora_adapter.safetensors (tiny delta, ~50MB)
    
      They are merged at inference OR kept separate.
    
    
    SCENARIO 3 — ACTUALLY ADDING HEADS  (rare, research territory)
      ───────────────────────────────────────────────────────────────
    
      This IS possible but it is not "fine-tuning"
      It is building a NEW model that inherits old weights.
    
      Step 1: define new architecture (more heads)
      ┌────────────────────────────────────────────┐
      │  Old architecture:  32 heads, d_model=4096 │
      │  New architecture:  40 heads, d_model=4096 │
      │                                            │
      │  Old W_Q:  4096 × 4096                     │
      │  New W_Q:  4096 × 5120  ← different shape  │
      └────────────────────────────────────────────┘
    
      Step 2: initialise new model weights
    
      ┌────────────────────────────────────────────┐
      │  New W_Q (4096 × 5120)                     │
      │                                            │
      │  [  OLD W_Q VALUES   | NEW RANDOM COLS  ]  │
      │  [  (4096 × 4096)    | (4096 × 1024)   ]   │
      │   ↑                    ↑                   │
      │   copied from          randomly            │
      │   old model            initialised         │
      └────────────────────────────────────────────┘
    
      Step 3: retrain on dataset
      Backpropagation updates ALL values including
      the copied old ones and the new random ones.
    
      Step 4: result is a BRAND NEW model file
      Not a fine-tuned version. A new model entirely.
      Old model file still exists separately.
    
    
    
    ╔══════════════════════════════════════════════════════════════════╗
    ║  SUMMARY — WHAT HAPPENS TO WEIGHTS IN EACH CASE                  ║
    ╚══════════════════════════════════════════════════════════════════╝
    
                         Original    New         Dataset
                         Weights     Weights     drives
                         Modified?   Created?    shape?
                         ─────────   ─────────   ──────
      Full fine-tuning     YES        NO          NO
                           (nudged    (same       (only
                            slightly)  matrices)   values)
    
      LoRA fine-tuning      NO        YES         NO
                           (frozen)   (A and B    (only
                                      adapters)   A and B
                                                  values)
    
      Adding heads /        YES       YES         NO
      architecture change  (copied +  (new cols   (shape set
                            retrained) randomly    by architect
                                      init)       not data)
    
      ─────────────────────────────────────────────────────────────
      In ALL cases: dataset size never determines weight SHAPE.
      Dataset content determines weight VALUES.
      Shape is always an architectural decision made upfront.
    
    
The mental model to hold: think of the weight matrix like a spreadsheet with a fixed number of rows and columns. 
Fine-tuning changes the numbers inside the cells. Adding heads means building a new spreadsheet with more columns — 
you can copy the old columns in, but it is a new spreadsheet, not the old one with extras appended.
...
## SESSION ISOLATION — Session-Scoped vs. Persistent Memory

A persistent question from new practitioners: "Does the model remember our last conversation?"
The short answer is no — and understanding precisely why reveals important truths about how LLMs work.

    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHAT EXISTS DURING A SINGLE SESSION (one API call)              ║
    ╚══════════════════════════════════════════════════════════════════╝

      Inside a single forward pass + decode loop:

      ┌─────────────────────────────────────────────────────────────┐
      │  LIVES DURING THIS SESSION ONLY                             │
      │                                                             │
      │  • The context window contents (all tokens so far)          │
      │  • The KV cache (K and V matrices for all processed tokens)  │
      │  • Q, K, V vectors (exist only during the forward pass)     │
      │  • Intermediate activations (exist for microseconds)        │
      │  • Logits (exist until sampling completes)                  │
      │                                                             │
      │  PERSISTS FOREVER (unchanged by any conversation)           │
      │                                                             │
      │  • All model weights: W_Q, W_K, W_V, W_FFN, etc.           │
      │  • The embedding table                                      │
      │  • The tokeniser vocabulary and merge rules                 │
      └─────────────────────────────────────────────────────────────┘

      When the session ends (API call completes):

      ┌─────────────────────────────────────────────────────────────┐
      │  DISCARDED immediately                                      │
      │                                                             │
      │  • The KV cache — all K/V matrices freed from GPU memory   │
      │  • Q vectors — already gone during the forward pass        │
      │  • Activations — already gone                              │
      │  • The context window — not retained anywhere in the model │
      │                                                             │
      │  NOTHING about this conversation modified the weights.     │
      │  The model is IDENTICAL to before you started talking.     │
      └─────────────────────────────────────────────────────────────┘


    ╔══════════════════════════════════════════════════════════════════╗
    ║  TIMELINE ACROSS THREE SEPARATE API CALLS                        ║
    ╚══════════════════════════════════════════════════════════════════╝

      STATE OF GPU BEFORE SESSION 1
      ──────────────────────────────
      VRAM:     [ W_Q | W_K | W_V | W_FFN | Embed | ... ]  ← all frozen
      KV cache: empty

      SESSION 1  ("What is Paris?")
      ──────────────────────────────
      • Tokens assembled: [SYS][User: "What is Paris?"]
      • Pre-fill: KV cache fills with KV for all prompt tokens
      • Decode loop: generates "Paris is the capital of France."
      • Session ends: KV cache CLEARED
      • Weights: unchanged

      STATE OF GPU AFTER SESSION 1
      ──────────────────────────────
      VRAM:     [ W_Q | W_K | W_V | W_FFN | Embed | ... ]  ← identical to before
      KV cache: empty  ← no trace of Session 1 remains

      SESSION 2  ("What did we just discuss?")
      ─────────────────────────────────────────
      • Tokens assembled: [SYS][User: "What did we just discuss?"]
      • The model has NO access to Session 1's content
      • It cannot answer — unless Session 1's text was explicitly
        injected into this session's context window by the application
      • Session ends: KV cache CLEARED again


    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHERE "MEMORY" ACTUALLY LIVES IN PRODUCTION SYSTEMS             ║
    ╚══════════════════════════════════════════════════════════════════╝

      The model has no memory. Memory is an APPLICATION LAYER concern.

      OPTION 1 — Full conversation history re-sent each turn
      ───────────────────────────────────────────────────────
      Application stores prior turns and appends them to every new prompt.
      The model "remembers" only because its current context contains the
      full history. This is how ChatGPT-style multi-turn interfaces work.

      [SYS][Turn1_User][Turn1_Asst][Turn2_User][Turn2_Asst][New_User]
       ↑ always re-sent in full                              ↑ new input

      Cost: grows linearly — a 20-turn conversation has 20× more tokens.
      Limit: eventually the context window fills up and old turns are dropped.


      OPTION 2 — Summarisation / compression
      ─────────────────────────────────────────
      After N turns, a background call summarises the conversation.
      The summary replaces the raw history in the system prompt.
      Trades precision for context efficiency.


      OPTION 3 — Vector database retrieval (RAG)
      ─────────────────────────────────────────
      Past conversations stored as embeddings in a vector database.
      At each new turn, the most semantically relevant past exchanges
      are retrieved and injected into the current context window.

      Key insight: in ALL cases, the model itself has no memory.
      The application layer creates the illusion of continuity.
      The weights never change between conversations.


---


## INFERENCE MENTAL MODEL — Correcting the Threshold Misconception

The most common wrong mental model brought to LLMs is borrowed from biology:
"neurons fire when they receive enough signal." In a biological neuron there IS
a threshold — a binary decision: fire or don't fire. The LLM neuron works
entirely differently.

    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE WRONG MODEL  (biological threshold)                         ║
    ╚══════════════════════════════════════════════════════════════════╝

      What people imagine:

      inputs ──▶ [THRESHOLD CHECK] ──▶  0  (silent, does not fire)
                                        1  (fires!)

      i.e.: if total input signal > threshold → neuron activates;
            otherwise silent.

      This produces a BINARY, DISCRETE decision at every neuron.

      Why this feels intuitive: it matches logic gates, decision trees,
      and the popular description of biological brain cells.

      WHY IT IS WRONG FOR LLMs:

      LLM neurons are NOT binary. They output a continuous real number.
      There is no threshold. The output is never "fire or don't fire."
      A neuron outputting 0.3 is not "silent" — it is contributing 0.3
      to every neuron in the next layer.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE CORRECT MODEL  (weighted continuous transformation)          ║
    ╚══════════════════════════════════════════════════════════════════╝

      inputs × weights  →  sum  →  add bias  →  activation  →  output

      At no point is there a binary decision.
      The activation function introduces nonlinearity — it bends the
      curve — but it is smooth and continuous at almost every point.

      Example with ReLU (the simplest common activation):

        pre_activation = -2.3  →  ReLU output:  0.0   (clamped at zero)
        pre_activation = -0.1  →  ReLU output:  0.0   (clamped at zero)
        pre_activation =  0.0  →  ReLU output:  0.0   (exactly zero)
        pre_activation =  0.1  →  ReLU output:  0.1   (passes through)
        pre_activation =  3.7  →  ReLU output:  3.7   (passes through)

      ┌─────────────────────────────────────────────────────────────┐
      │  ReLU:   output = max(0, pre_activation)                    │
      │                                                             │
      │  output ↑              /                                    │
      │          |            /                                     │
      │          |           /                                      │
      │          |          /                                       │
      │          |_________/________________________ pre_activation │
      │          ↑                ↑                                 │
      │       negative         positive                             │
      │       → zero           → passes through unchanged          │
      └─────────────────────────────────────────────────────────────┘

      This is NOT "fire or don't fire." The neuron always produces an output.
      When the pre-activation is negative, that output happens to be 0.
      But 0 is a real number that propagates to every neuron in the next layer.
      It is not silence — it is signal with value zero.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE PATH-SELECTION ANALOGY — WHAT IS RIGHT AND WRONG            ║
    ╚══════════════════════════════════════════════════════════════════╝

      Some explanations describe transformers as "selecting paths" through
      the network — as if the model routes tokens down different branches.

      WHAT IS CORRECT in this analogy:

        • Attention weights DO emphasise some token connections over others.
          Token A attending heavily to Token B and barely to Token C is
          genuinely selective — it is path-like in a loose sense.

        • Activation functions DO suppress some neuron outputs to zero
          (especially ReLU). A neuron with zero output does not contribute
          to the next layer — functionally inactive for that input.

        • Different inputs activate DIFFERENT sets of neurons to non-zero
          values. In this sense, different inputs do light up different
          parts of the network.

      WHAT IS WRONG in this analogy:

        • The network structure does NOT change. There are no branches,
          no routing switches, no if-statements. Every input passes through
          IDENTICAL weight matrices every single time. Nothing is bypassed.

        • "Path selection" implies discrete decision points with clear branch
          conditions. What actually happens is a smooth, continuous geometric
          transformation of a high-dimensional vector.

        • The model is not "choosing" anything until the final sampling step.
          Before sampling, every computation is deterministic matrix arithmetic
          followed by a continuous activation function.

      THE CORRECT MENTAL MODEL IN ONE SENTENCE:

          A transformer inference call is a fixed sequence of matrix
          multiplications and smooth nonlinearities applied to a token
          vector — no discrete decisions, no routing, no search —
          until the final softmax-and-sampling step picks one token.


---


## SINGLE NEURON — Weights, Bias, Activation, Output

This section zooms in to the atomic unit of computation before scaling up
to layers and the full network.

    ╔══════════════════════════════════════════════════════════════════╗
    ║  ANATOMY OF ONE NEURON                                           ║
    ╚══════════════════════════════════════════════════════════════════╝

      A single neuron receives N inputs and produces 1 output.
      It has exactly two kinds of learnable parameters:

        • Weights  w₁, w₂, ..., wₙ  (one per input)
        • Bias     b                (one scalar offset)

      INPUTS  →  WEIGHTED SUM  →  ADD BIAS  →  ACTIVATION  →  OUTPUT

        x₁ ──w₁──┐
        x₂ ──w₂──┤
        x₃ ──w₃──┼──▶  z = (x₁w₁ + x₂w₂ + ... + xₙwₙ) + b  ──▶  a = f(z)
        ...       │
        xₙ ──wₙ──┘

      Where:
        z  = pre-activation value  (raw weighted sum plus bias)
        f  = activation function   (ReLU, SiLU, GeLU, etc.)
        a  = activation output     (what this neuron sends to the next layer)


    ╔══════════════════════════════════════════════════════════════════╗
    ║  WORKED EXAMPLE — ONE NEURON, TWO DIFFERENT INPUTS               ║
    ╚══════════════════════════════════════════════════════════════════╝

      Scenario: one neuron in the FFN hidden layer of a 4-dimensional
      transformer (simplified — real models use 4096 dimensions).
      This neuron receives the 4-element token vector.

      WEIGHTS FOR THIS NEURON (learned during training, then frozen):
        w₁ = 0.5,   w₂ = -0.2,   w₃ = 0.8,   w₄ = 0.3
        b  = 0.1

      ── INPUT: "cat" vector ──────────────────────────────────────────

        x₁ = 0.9,   x₂ = 0.1,   x₃ = 0.4,   x₄ = -0.3

        z = (0.9 × 0.5) + (0.1 × -0.2) + (0.4 × 0.8) + (-0.3 × 0.3) + 0.1
          =   0.45       +   -0.02      +    0.32      +   -0.09       + 0.1
          =   0.76

        a = ReLU(0.76) = 0.76   ← positive, passes through unchanged

        This neuron sends 0.76 to EVERY neuron in the next layer.

      ── INPUT: "the" vector ──────────────────────────────────────────

        x₁ = 0.2,   x₂ = 0.5,   x₃ = -0.1,   x₄ = 0.8

        z = (0.2 × 0.5) + (0.5 × -0.2) + (-0.1 × 0.8) + (0.8 × 0.3) + 0.1
          =   0.10       +   -0.10       +   -0.08       +   0.24      + 0.1
          =   0.26

        a = ReLU(0.26) = 0.26

      Same weights. Different input. Different output.
      This is how the same FFN processes different tokens differently —
      not by changing its weights, but because different inputs produce
      different weighted sums through identical parameters.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHAT THE WEIGHTS AND BIAS ENCODE                                 ║
    ╚══════════════════════════════════════════════════════════════════╝

      Each weight wᵢ encodes how much input dimension i matters
      for this particular neuron's function:

        Large positive wᵢ  → input dimension i strongly activates this neuron
        Large negative wᵢ  → input dimension i suppresses this neuron
        Near-zero wᵢ       → input dimension i is mostly ignored

      The bias b shifts the activation threshold:

        Negative b  → neuron is harder to activate (needs stronger input signal)
        Positive b  → neuron activates even with weak input signal
        Zero b      → neuron fires at zero exactly when all weighted inputs cancel

      During training, backpropagation adjusts every weight and bias
      across all neurons in all layers simultaneously, nudging each
      parameter by a tiny amount (the learning rate × gradient) in the
      direction that reduces the loss. After training, all values are frozen.
      The model file on disk is simply the complete collection of all these
      frozen numbers.
...
## NEURON CONNECTIVITY — Inputs, Outputs, Fan-Out Structure

    ╔══════════════════════════════════════════════════════════════════╗
    ║  FROM ONE NEURON TO A FULLY CONNECTED LAYER                      ║
    ╚══════════════════════════════════════════════════════════════════╝

      The FFN inside each transformer layer is a FULLY CONNECTED
      (dense) two-layer network.

      "Fully connected" means: every neuron in layer L sends its output
      to EVERY neuron in layer L+1. No selective routing. No sparsity.
      Complete all-to-all connectivity.

      Example with d_model=4 inputs, d_ff=8 hidden neurons (simplified):

        INPUT (4 neurons)          HIDDEN (8 neurons)
      ...
      Every one of the 4 input neurons connects to all 8 hidden neurons.
      Connections in this layer: 4 × 8 = 32 weight values.

      Then the 8 hidden neurons each connect to all 4 output neurons:
      Connections: 8 × 4 = 32 more weight values.

      In matrix notation: this is a single matrix multiply  X · W
      where W has shape (d_model × d_ff) = (4 × 8) = 32 parameters.

      In a real 7B-parameter model:
        d_model = 4096,   d_ff = 14336  (LLaMA 3 8B, SwiGLU)
        Layer 1 weights:  4096 × 14336 = 58,720,256  (~59M per layer)
        Gate weights:     4096 × 14336 = 58,720,256  (~59M per layer, SwiGLU)
        Layer 2 weights:  14336 × 4096 = 58,720,256  (~59M per layer)
        Per-layer FFN total: ~176M weights
        × 32 layers = ~5.6 billion weights just in FFN layers
    ...
    ╚══════════════════════════════════════════════════════════════════╝
      ...
      FAN-IN  = number of inputs a single neuron receives
                = width of the PREVIOUS layer

        In the first FFN layer (d_model → d_ff):
          Each hidden neuron receives d_model = 4096 inputs.
          Fan-in = 4096.

        In the second FFN layer (d_ff → d_model):
          Each output neuron receives d_ff = 14336 inputs.
          Fan-in = 14336.


      FAN-OUT = number of neurons a single neuron feeds into
                = width of the NEXT layer

        Each input neuron sends its output to all d_ff hidden neurons.
        Fan-out = d_ff = 14336.

        Each hidden neuron sends its output to all d_model output neurons.
        Fan-out = d_model = 4096.


      WHY FAN-IN MATTERS FOR WEIGHT INITIALISATION:

      If a neuron has fan-in = 4096 and each input is roughly ~1.0,
      the pre-activation sum z would be ~4096 before weights are applied.
      Weights initialised too large → z explodes.
      Weights initialised too small → z vanishes to zero.

      Standard fix: initialise weights with standard deviation 1/√(fan-in).
      This keeps z in a stable range regardless of layer width.

        He initialisation (ReLU layers):  std = √(2 / fan_in)
        Xavier initialisation:            std = √(2 / (fan_in + fan_out))

      Even with good initialisation, values drift layer by layer.
      This is precisely why Layer Norm / RMS Norm exists — covered next.


---


## INPUT CONNECTION LIMITS — Who Sets Layer Width and Why

    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE ARCHITECTURAL HYPERPARAMETERS                               ║
    ╚══════════════════════════════════════════════════════════════════╝

      The width of every layer in a transformer is determined by a small
      set of hyperparameters decided during architecture design — BEFORE
      training begins. They cannot be changed without retraining from scratch.

      ┌─────────────────┬───────────────────────────────────────────────┐
      │  Parameter      │  What it controls                             │
      ├─────────────────┼───────────────────────────────────────────────┤
      │  d_model        │  Width of every token vector throughout the   │
      │                 │  entire model. Also called "hidden size" or   │
      │                 │  "embedding dimension". Every layer input and  │
      │                 │  output has exactly this many values.         │
      ├─────────────────┼───────────────────────────────────────────────┤
      │  d_ff           │  Width of the hidden layer inside the FFN     │
      │                 │  sub-layer. Typically 4 × d_model for plain   │
      │                 │  MLP; ~2.67 × d_model for SwiGLU (to keep    │
      │                 │  parameter count comparable).                 │
      ├─────────────────┼───────────────────────────────────────────────┤
      │  n_heads        │  Number of attention heads. Each head operates │
      │                 │  on a slice of d_model / n_heads dimensions.  │
      ├─────────────────┼───────────────────────────────────────────────┤
      │  n_layers       │  Depth: how many transformer blocks are       │
      │                 │  stacked sequentially.                        │
      ├─────────────────┼───────────────────────────────────────────────┤
      │  vocab_size     │  Width of the embedding table and the LM head │
      │                 │  output projection.                           │
      └─────────────────┴───────────────────────────────────────────────┘
    ...
    ╚══════════════════════════════════════════════════════════════════╝
      ...
      ┌──────────────────────┬────────┬──────────┬─────────┬──────────┐
      │  Model               │d_model │  d_ff    │ n_heads │ n_layers │
      ├──────────────────────┼────────┼──────────┼─────────┼──────────┤
      │  GPT-2 Small (117M)  │  768   │   3072   │   12    │    12    │
      │  GPT-2 XL  (1.5B)    │  1600  │   6400   │   25    │    48    │
      │  LLaMA 3  (8B)       │  4096  │  14336   │   32    │    32    │
      │  LLaMA 3  (70B)      │  8192  │  28672   │   64    │    80    │
      │  LLaMA 3  (405B)     │  16384 │  53248   │  128    │   126    │
      └──────────────────────┴────────┴──────────┴─────────┴──────────┘

      Observation: d_ff / d_model ≈ 3.5 in all LLaMA 3 models —
      this is the SwiGLU ratio (vs 4.0 for a plain ReLU FFN).


    ╔══════════════════════════════════════════════════════════════════╗
    ║  WHY NOT MAKE EVERY LAYER INFINITELY WIDE?                       ║
    ╚══════════════════════════════════════════════════════════════════╝

      Three hard constraints prevent this:

      1. PARAMETERS → MEMORY
         FFN parameters per layer = 3 × d_model × d_ff  (SwiGLU)
         Doubling d_model increases FFN params by 4× (both dims scale).

         A 7B model  → ~14 GB in fp16 just for weights
         A 70B model → ~140 GB
         There is a hard ceiling of GPU VRAM physically available.

      2. COMPUTE → SPEED AND COST
         Each matrix multiply cost scales with (input_width × output_width).
         Wider layers = more multiply-accumulate operations per token.
         A 2× wider model takes ~4× longer per layer.

      3. DIMINISHING RETURNS (SCALING LAWS)
         Empirically (Chinchilla et al., 2022), capability improves as a
         smooth power law with parameter count. Doubling parameters never
         doubles capability. Beyond a point, adding width is less efficient
         than adding depth or adding training data.

         There is an optimal ratio of width, depth, and training tokens
         for any given compute budget. Arbitrarily wide layers are not
         the optimal use of that budget.

      WHO DECIDES:

      The architecture team sets these values at design time based on:
        • Available training compute budget (GPU-hours)
        • Target deployment memory footprint
        • Inference latency and throughput requirements
        • Scaling law estimates for expected quality at that parameter count

      Once baked into the model file, they are immutable.
      Every inference call, every user, every session — the same widths.


---


## PARALLEL PROCESSING — Simultaneous vs. Sequential Aggregation

    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE BIG PICTURE: WHAT RUNS IN PARALLEL VS. SEQUENTIALLY         ║
    ╚══════════════════════════════════════════════════════════════════╝

      ┌────────────────────────────────────┬──────────────────────────┐
      │  Parallel (all at once)            │  Sequential (must wait)  │
      ├────────────────────────────────────┼──────────────────────────┤
      │  All tokens during pre-fill        │  Each decode step        │
      │  All attention heads per layer     │  Each transformer layer  │
      │  All neurons within a layer        │  Each generated token    │
      │  All matrix columns simultaneously │  (depends on prev token) │
      │  All sequences in a batch          │                          │
      └────────────────────────────────────┴──────────────────────────┘


    ╔══════════════════════════════════════════════════════════════════╗
    ║  WITHIN A SINGLE LAYER — ALL NEURONS FIRE SIMULTANEOUSLY         ║
    ╚══════════════════════════════════════════════════════════════════╝

      Inside the FFN sub-layer, every neuron in the hidden layer
      independently computes its weighted sum at the exact same time.

      For d_model=4 inputs and d_ff=8 hidden neurons:

        Neuron 1:  z₁ = (x₁w₁₁) + (x₂w₁₂) + (x₃w₁₃) + (x₄w₁₄) + b₁
        Neuron 2:  z₂ = (x₁w₂₁) + (x₂w₂₂) + (x₃w₂₃) + (x₄w₂₄) + b₂
        Neuron 3:  z₃ = ...
        ...
        Neuron 8:  z₈ = ...

      All 8 of these sums are computed by the GPU simultaneously —
      not "neuron 1 waits for neuron 2." This is exactly what a
      matrix multiply is: X · W computes all output neurons at once.

      The activations [a₁, a₂, ..., a₈] are also applied element-wise,
      simultaneously, in a single GPU kernel.

      In a real 7B model: 14,336 neurons in the hidden layer all fire
      at the same instant for every token on every forward pass.
      Then 4,096 output neurons in layer 2 all compute simultaneously.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  ACROSS LAYERS — STRICTLY SEQUENTIAL                             ║
    ╚══════════════════════════════════════════════════════════════════╝

      Layer 2 cannot start until Layer 1 is fully complete.
      Layer 1's output vector IS the input to Layer 2.

      Layer 1  →  Layer 2  →  Layer 3  →  ...  →  Layer 32
      ↑
      must finish before Layer 2 can begin any computation

      This sequential dependency is irreducible.
      It is why model depth (n_layers) adds latency in a way that
      width (d_model, d_ff) does not — each layer adds one serial step.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  PRE-FILL — ALL TOKENS PROCESSED SIMULTANEOUSLY                  ║
    ╚══════════════════════════════════════════════════════════════════╝

      During pre-fill, all N prompt tokens are stacked into one matrix
      and processed together in a single batched forward pass.

      For "The cat sat on the mat" (6 tokens):

        X shape: (6 × 4096)  — all 6 token vectors in one matrix

        FFN Layer 1:   X · W₁  →  (6 × 14336)
                                   ↑ all 6 tokens' hidden neurons computed at once

        Attention:     Q = X · W_Q  →  (6 × heads × d_k)   ← all tokens
                       K = X · W_K  →  (6 × heads × d_k)   ← all tokens
                       scores = Q · Kᵀ  →  (6 × 6 matrix)  ← all pairs

      Why pre-fill is fast: every token's computation at every layer
      is fully parallelised. The GPU runs at peak utilisation.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  DECODE — ONE TOKEN AT A TIME (THE SEQUENTIAL BOTTLENECK)        ║
    ╚══════════════════════════════════════════════════════════════════╝

      Once generation begins, each token depends on all prior tokens.
      Token N+1 cannot be generated until token N is known.

      ────────────────────────────────────────────────────▶ time
      Step 1:  generate token 4   (needs tokens 1–3)
      Step 2:  generate token 5   (needs tokens 1–4)  ← waits for step 1
      Step 3:  generate token 6   (needs tokens 1–5)  ← waits for step 2

      Each decode step processes exactly one new token through all 32 layers.
      The GPU is severely under-utilised during decode.

      ┌─────────────────────────────────────────────────────────────┐
      │  PRE-FILL vs DECODE — GPU utilisation                       │
      │                                                             │
      │  Pre-fill:  [████████████████████████]  ~95% GPU active    │
      │             N tokens processed in ONE parallel pass         │
      │                                                             │
      │  Decode:    [███░░░░░░░░░░░░░░░░░░░░░]  ~10% GPU active    │
      │             1 token processed per step                      │
      └─────────────────────────────────────────────────────────────┘

      This is the fundamental reason speculative decoding (Section 8),
      continuous batching, and large batch sizes all exist: they each
      attempt to improve GPU utilisation during the inherently
      sequential decode phase.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  ATTENTION HEADS — PARALLEL SPECIALISATION                       ║
    ╚══════════════════════════════════════════════════════════════════╝

      In multi-head attention, all H heads compute simultaneously.
      Each head has its own W_Q, W_K, W_V projection matrices.
      All heads run their complete attention computation in parallel.
      Their outputs are concatenated and projected back to d_model via W_O.

      Head 1 learns to attend to → subject/verb agreement
      Head 2 learns to attend to → positional proximity
      Head 3 learns to attend to → coreference (pronoun → noun)
      ...    (each head specialises during training into different patterns)

      All H=32 heads compute at the same time on the same input tensor.
      Parallelism is both within each head (all tokens simultaneously)
      and across heads (all heads simultaneously).


---


## INPUT SCALE BOUNDS — Layer Norm, Value Bounds, Activation Functions

    ╔══════════════════════════════════════════════════════════════════╗
    ║  THE PROBLEM: EXPLODING AND VANISHING VALUES                     ║
    ╚══════════════════════════════════════════════════════════════════╝

      Without normalisation, values grow or shrink exponentially as they
      pass through 32+ transformer layers.

      Consider a value of 1.5 where each layer multiplies it by ~1.2 on average:

        After layer 1:   1.5 × 1.2¹  =   1.80
        After layer 5:   1.5 × 1.2⁵  =   3.73
        After layer 10:  1.5 × 1.2¹⁰ =   9.29
        After layer 20:  1.5 × 1.2²⁰ =  57.50
        After layer 32:  1.5 × 1.2³² = 629.00  ← attention scores collapse

      The reverse (vanishing): if the average multiplier is 0.8:

        After layer 32:  1.5 × 0.8³² = 0.001  ← gradients vanish, early layers learn nothing

      Consequence without normalisation:
        • Attention scores become meaningless (softmax saturates to one-hot)
        • Logits are dominated by noise
        • Training diverges; no stable gradient signal reaches early layers

      SOLUTION: normalise at each layer to keep values bounded throughout.


    ╔══════════════════════════════════════════════════════════════════╗
    ║  LAYER NORMALISATION — TWO VARIANTS                              ║
    ╚══════════════════════════════════════════════════════════════════╝

      LAYER NORM (original — used in BERT, GPT-2):

        Given a vector h of d_model values for one token:

          1. Mean:        μ  = (1/d) × Σ hᵢ
          2. Variance:    σ² = (1/d) × Σ (hᵢ - μ)²
          3. Normalise:   ĥᵢ = (hᵢ - μ) / √(σ² + ε)
          4. Rescale:     outputᵢ = γᵢ × ĥᵢ + βᵢ

        γ (gamma) and β (beta) are LEARNED per-dimension parameters.
        The model decides how much to re-scale and re-centre after normalisation.
        ε ≈ 1e-5 prevents division by zero.

        Result: output has zero mean and unit variance, then is linearly
        rescaled by the learned γ and β. Shape is preserved (same d_model).


      RMS NORM (modern standard — LLaMA, Mistral, Gemma):

        Simpler: skips the mean subtraction entirely.

          1. RMS:        RMS(h) = √((1/d) × Σ hᵢ²)
          2. Normalise:  ĥᵢ = hᵢ / RMS(h)
          3. Rescale:    outputᵢ = γᵢ × ĥᵢ

        No β term — only γ is learned.
        Slightly faster. Empirically matches or exceeds Layer Norm quality.
        Default in all LLaMA-family and most post-2022 models.


      WHERE NORMALISATION IS APPLIED IN A TRANSFORMER BLOCK:

      ┌──────────────────────────────────────────────────────────────┐
      │  Pre-LN (modern standard — LLaMA, Mistral, likely GPT-4):   │
      │                                                              │
      │  x ──┬─────────────────────────────────────────────┐        │
      │      │                                             │        │
      │      ▼                                             │        │
      │  [RMS Norm] → [Self-Attention] ──▶ (+) ────────────┘        │
      │      ↑                             ↑  residual add          │
      │  normalise BEFORE attention        └── skip connection       │
      │                                                              │
      │      │                                                       │
      │      ▼                                                       │
      │  x' ─┬─────────────────────────────────────────────┐        │
      ...
      │                                    ↑  residual add          │
      │                                                              │
      │  Pre-LN is more stable for training very deep networks.     │
      │  The skip connection ensures the original signal always      │
      │  flows through, even if the sub-layer output is noisy.      │
      └──────────────────────────────────────────────────────────────┘


    ╔══════════════════════════════════════════════════════════════════╗
    ║  ACTIVATION FUNCTIONS — OUTPUT RANGES AND BEHAVIOUR              ║
    ╚══════════════════════════════════════════════════════════════════╝

      ┌──────────────┬──────────────────────────┬──────────────────────────────────────┐
      │  Function    │  Formula                 │  Output Range / Notes                │
      ├──────────────┼──────────────────────────┼──────────────────────────────────────┤
      │  ReLU        │  max(0, x)               │  [0, ∞)  hard zero below 0           │
      │              │                          │  Simple, fast. Dead neuron risk.      │
      ├──────────────┼──────────────────────────┼──────────────────────────────────────┤
      │  GeLU        │  x · Φ(x)               │  ≈(-0.17, ∞)  smooth near zero       │
      │              │  Φ = Gaussian CDF        │  Used in BERT, GPT-2, GPT-3.         │
      │              │                          │  Differentiable everywhere.           │
      ├──────────────┼──────────────────────────┼──────────────────────────────────────┤
      │  SiLU/Swish  │  x · σ(x)               │  ≈(-0.28, ∞)  smooth, no hard zero   │
      │              │  σ = sigmoid function    │  Slight negative region allowed.     │
      ├──────────────┼──────────────────────────┼──────────────────────────────────────┤
      │  SwiGLU      │  SiLU(x·W_gate) ⊙        │  Unbounded positive. Gated product.  │
      │              │  (x · W_value)           │  Used in LLaMA, PaLM, Gemini.        │
      │              │  (⊙ = element-wise mul)  │  Requires two weight matrices.       │
      └──────────────┴──────────────────────────┴──────────────────────────────────────┘


    ╔══════════════════════════════════════════════════════════════════╗
    ║  SWIGLU IN DEPTH — WHY MODERN MODELS REPLACED RELU               ║
    ╚══════════════════════════════════════════════════════════════════╝

      SwiGLU is a gated activation: it multiplies two parallel linear
      projections together, where one acts as a smooth sigmoid-shaped gate.

      Standard FFN (with ReLU):

        h = ReLU(X · W₁ + b₁)          ← one projection, one activation
        output = h · W₂ + b₂

      SwiGLU FFN (LLaMA, PaLM, Gemini):

        gate  = SiLU(X · W_gate)        ← smooth gate (values between -0.28 and ∞)
        value = X · W_value             ← plain linear transform
        h     = gate ⊙ value            ← element-wise product (gating)
        output = h · W_out

      ┌──────────────────────────────────────────────────────────────┐
      │  INTUITION FOR GATING:                                       │
      │                                                              │
      │  gate ≈ 1.0  →  value passes through nearly unchanged       │
      │  gate ≈ 0.0  →  value is suppressed to near zero            │
      │  gate ≈ 0.5  →  value is halved                             │
      │                                                              │
      │  Unlike ReLU's hard cutoff, SwiGLU's gate is smooth.        │
      │  The model gains fine-grained, context-dependent control     │
      │  over which features to amplify and which to suppress.       │
      └──────────────────────────────────────────────────────────────┘

      WHY SWIGLU OUTPERFORMS RELU IN PRACTICE:

        • Smooth gradients everywhere → more stable training
        • No hard zero: negative pre-activations still carry gradient,
          so neurons never get permanently "stuck at zero" (the
          "dying ReLU" problem where a neuron outputs 0 for every input
          after a large negative weight update)
        • Gating allows conditional computation: the model learns to
          suppress irrelevant dimensions depending on context

      PARAMETER COST:
        SwiGLU requires W_gate AND W_value instead of one W₁.
        To keep parameter count comparable to a ReLU FFN at d_ff = 4 × d_model,
        SwiGLU is run at d_ff ≈ 2.67 × d_model.
        LLaMA 3 8B: d_ff = 14336 ≈ 3.5 × 4096 (a common practical choice).


    ╔══════════════════════════════════════════════════════════════════╗
    ║  VALUE BOUNDS TRACED THROUGH A FULL TRANSFORMER LAYER            ║
    ╚══════════════════════════════════════════════════════════════════╝

      Token vector enters layer (after the previous layer's residual):

        h = [0.8, -0.3, 1.2, 0.4, ...]   ← d_model values
                                            typical magnitude: most values between -3 and +3

              ↓  RMS Norm  (pre-attention)
        RMS(h) ≈ 0.7
        ĥ = h / 0.7 = [1.14, -0.43, 1.71, 0.57, ...]
        then multiplied by learned γ → values re-scaled, still bounded

              ↓  Self-Attention
        Attention scores (Q·Kᵀ / √d_k) are also normalised by √d_k
        before softmax — preventing the dot products from growing
        proportionally to d_k and saturating the softmax.
        Output has same shape d_model. Residual adds original h back.

              ↓  RMS Norm again  (pre-FFN)
        Re-normalises. Resets any drift from the attention sub-layer.

              ↓  SwiGLU FFN
        Expands to d_ff, applies gating, projects back to d_model.
        Residual adds pre-FFN value back.
        Range: approximately [-10, +10] — bounded, not exploding.

      Without the two RMS Norm operations per layer, values would drift
      into the hundreds by layer 10 and into numerical overflow by layer 32.
      With them, the model trains successfully to 32, 80, and 126 layers.
