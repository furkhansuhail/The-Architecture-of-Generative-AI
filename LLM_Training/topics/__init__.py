...
def get_all_topics() -> dict:
    ...
    for py_file in sorted(pkg_dir.glob("*.py")):
        if py_file.name.startswith("_"):
            continue
        module_name = f"LLM_Training.topics.{py_file.stem}"
        try:
            mod = importlib.import_module(module_name)
        except Exception as e:
            import warnings
            warnings.warn(f"[LLM_Training.topics] Could not import '{module_name}': {e}", stacklevel=2)
            continue
        ...
        try:
            ...
            import warnings
            warnings.warn(f"[LLM_Training.topics] get_content() failed in '{module_name}': {e}", stacklevel=2)
            continue
