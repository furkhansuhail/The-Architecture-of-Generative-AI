...
Not every subset of Ω can be assigned a consistent probability — for
example, on the real line there exist non-measurable sets (Vitali sets)
to which no area can be assigned without contradiction.

A σ-ALGEBRA (or σ-field) ℱ is a collection of subsets of Ω satisfying:

    1. Ω ∈ ℱ                                (whole space is measurable)
    2. A ∈ ℱ  →  Aᶜ ∈ ℱ                    (closed under complement)
    3. A₁, A₂, ... ∈ ℱ  →  ⋃ᵢ Aᵢ ∈ ℱ      (closed under countable union)

These closure properties also give:  ∅ ∈ ℱ,  and ℱ is closed under
countable intersections (by De Morgan's laws).

The triple (Ω, ℱ, P) is a PROBABILITY SPACE.

    Smallest σ-algebra on Ω:    {∅, Ω}     (trivial — no information)
    Largest σ-algebra on Ω:     2^Ω        (all subsets — fine for discrete Ω)

BOREL σ-ALGEBRA B(ℝ): The σ-algebra generated by all open intervals on ℝ.
This is the standard σ-algebra for continuous probability.  It contains
all open sets, closed sets, countable intersections and unions thereof —
essentially every set you would ever want to assign a probability to.

MEASURABLE FUNCTION: A random variable X: Ω → ℝ is measurable w.r.t.
ℱ if  {ω ∈ Ω : X(ω) ≤ x} ∈ ℱ  for every x ∈ ℝ.
This is what makes P(X ≤ x) well-defined — the pre-image is in ℱ.

    Why this matters for ML:
    · Formal foundation for "which events have well-defined probability"
    · Conditional expectation E[X|ℱ'] is defined w.r.t. a sub-σ-algebra ℱ'
      — the information available at time t in a filtration (stochastic processes)
    · Measurability underpins Lebesgue integration and convergence theorems
...
The k-th MOMENT of X:   μₖ = E[Xᵏ] = ∫ xᵏ f(x)dx

The k-th CENTRAL MOMENT:  E[(X − μ)ᵏ]   where μ = E[X]

    k=1: E[X]         — mean (location)
    k=2: E[(X−μ)²]    — variance (spread)
    k=3: E[(X−μ)³]/σ³ — SKEWNESS  (asymmetry)
        > 0: right tail heavier (positive skew)
        < 0: left tail heavier (negative skew)
        = 0: symmetric (Gaussian, uniform)
    k=4: E[(X−μ)⁴]/σ⁴ — KURTOSIS  (tail weight)
        Gaussian kurtosis = 3  (reference)
        Excess kurtosis = kurtosis − 3
        > 0: heavier tails than Gaussian (leptokurtic — t-distribution, Laplace)
        < 0: lighter tails (platykurtic — uniform)

MOMENT GENERATING FUNCTION (MGF):

    M_X(t) = E[eᵗˣ]  (defined for t in a neighbourhood of 0)

    Key properties:
    · The k-th derivative at t=0 gives the k-th moment:
          M_X^(k)(0) = E[Xᵏ]
    · Uniqueness: if M_X(t) = M_Y(t) for all t near 0, then X and Y
      have the same distribution.
    · Sums: if X ⊥ Y then M_{X+Y}(t) = M_X(t) · M_Y(t)

    MGFs for key distributions:
        Bernoulli(p):    M(t) = 1−p + peᵗ
        Normal(μ,σ²):   M(t) = exp(μt + σ²t²/2)
        Poisson(λ):     M(t) = exp(λ(eᵗ−1))
        Exponential(λ): M(t) = λ/(λ−t)  for t < λ

    Limitation: MGF may not exist for all distributions (e.g. Cauchy has none).
...
The CHARACTERISTIC FUNCTION is the Fourier transform of the density:
    ...
    φ_X(t) = E[eⁱᵗˣ] = ∫ eⁱᵗˣ f(x)dx    (always exists — uses complex exponential)

    Key properties:
    · ALWAYS exists (unlike MGF), since |eⁱᵗˣ| = 1
    · φ_X(0) = 1
    · Uniqueness: φ_X = φ_Y  iff  X and Y have the same distribution
    · Independence: X ⊥ Y  iff  φ_{X+Y}(t) = φ_X(t) · φ_Y(t)
    · Moments: the k-th moment (if it exists) = i^{−k} φ_X^(k)(0)

    φ for key distributions:
        Normal(μ,σ²):  φ(t) = exp(iμt − σ²t²/2)
        Cauchy(0,1):   φ(t) = exp(−|t|)     (no finite moments!)

    ┌──────────────────────────────────────────────────────────┐
    │  CLT proof via characteristic functions:                  │
    │  φ_{X̄ₙ}(t) = [φ_X(t/√n)]ⁿ                                │
    │  Taylor expand: φ_X(t) ≈ 1 − σ²t²/2 + O(t³)             │
    │  → [1 − σ²t²/2n]ⁿ → exp(−σ²t²/2) = φ_{N(0,σ²)}(t)  ∎   │
    └──────────────────────────────────────────────────────────┘
...
**Multinomial(n, p₁,...,pₖ)**
    Models: n trials each landing in one of k categories with probs p₁,...,pₖ
    PMF:    P(X₁=x₁,...,Xₖ=xₖ) = n!/(x₁!···xₖ!) · p₁^x₁ ··· pₖ^xₖ
            where Σᵢ xᵢ = n and Σᵢ pᵢ = 1
    Mean:   E[Xᵢ] = npᵢ      Var: Var(Xᵢ) = npᵢ(1−pᵢ)
    Cov:    Cov(Xᵢ,Xⱼ) = −npᵢpⱼ  for i ≠ j  (negatively correlated!)
    Marginals: each Xᵢ ~ Binomial(n, pᵢ)
    Use in ML: multi-class classification output (paired with softmax + cross-entropy)
               topic models (LDA), language models (token distribution)

**Hypergeometric(N, K, n)**
    Models: drawing n items WITHOUT replacement from a population of N items,
            K of which are "successes"
    PMF:    P(X=k) = C(K,k)·C(N−K,n−k) / C(N,n)
    Mean:   nK/N        Var: nK(N−K)(N−n) / [N²(N−1)]
    Key: variance has the finite-population correction factor (N−n)/(N−1)
         → as N→∞: Hypergeometric → Binomial(n, K/N)  (sampling with replacement)
    Use in ML: evaluating classifiers without replacement (cross-validation draws),
               modelling selection bias, gene enrichment analysis
...
**Gamma(α, β)**
    Models: sum of α independent Exponential(β) random variables; waiting time
            until the α-th event in a Poisson process
    PDF:    f(x) = βᵅ xᵅ⁻¹ e^{−βx} / Γ(α)   for x > 0
    Mean:   α/β     Var: α/β²
    Shape parameter α, rate parameter β  (or scale θ=1/β)
    Special cases:
        Gamma(1, β)    = Exponential(β)
        Gamma(k/2, ½)  = Chi-squared(k)
        Gamma(α, β) with integer α = Erlang distribution
    Use in ML: conjugate prior for Poisson rate (Gamma-Poisson model),
               modelling positive-valued quantities, Bayesian inference for precision

**Chi-Squared(k)  — χ²(k)**
    Definition: If Z₁,...,Zₖ ~ iid N(0,1), then X = Σᵢ Zᵢ² ~ χ²(k)
    PDF:    f(x) = x^{k/2−1} e^{−x/2} / (2^{k/2} Γ(k/2))   for x > 0
    Mean:   k       Var: 2k      (skewed right for small k; → Normal as k→∞)
    Use in ML: goodness-of-fit tests (χ² test), likelihood ratio tests,
               distribution of sample variance: (n−1)S²/σ² ~ χ²(n−1)

**Student's t(ν)**
    Definition: T = Z / √(V/ν)  where Z ~ N(0,1) and V ~ χ²(ν) independent
    PDF:    f(x) ∝ (1 + x²/ν)^{−(ν+1)/2}
    Mean:   0 (for ν>1)      Var: ν/(ν−2) (for ν>2)    — heavier tails than Normal
    As ν → ∞:  t(ν) → N(0,1)   (Normal is the limit)
    Use in ML: small-sample inference (replacing Normal when σ² unknown),
               robust regression (t-distributed noise), Bayesian models

**F(d₁, d₂)**
    Definition: F = (U/d₁) / (V/d₂)  where U ~ χ²(d₁), V ~ χ²(d₂) independent
    Mean:   d₂/(d₂−2)  for d₂>2      (right-skewed, positive support)
    Use in ML: ANOVA F-test, overall significance test in regression,
               ratio of two sample variances under the Normal model

**Laplace(μ, b)**
    PDF:    f(x) = (1/2b) exp(−|x−μ|/b)    (double exponential)
    Mean:   μ       Var: 2b²     Excess kurtosis: 3  (heavier tails than Gaussian)
    CDF:    F(x) = ½ + ½ sign(x−μ)(1 − e^{−|x−μ|/b})
    MAP with Laplace prior = L1 (Lasso) regularisation
    Use in ML: L1 regularisation prior, robust noise model (MAE loss),
               sparse signal priors, Bayesian compressed sensing

**Cauchy(x₀, γ)**
    PDF:    f(x) = 1 / [πγ(1 + ((x−x₀)/γ)²)]
    Mean:   UNDEFINED    Var: UNDEFINED   (heavy tails — all moments infinite)
    Characteristic function: φ(t) = exp(ix₀t − γ|t|)
    Key fact: ratio of two independent N(0,1) variables is Cauchy(0,1)
    The law of large numbers FAILS — sample means do not converge!
    Use in ML: robustness research, heavy-tailed noise modelling,
               variational inference with fat-tailed priors
...
### Distribution Cheat Sheet
    ...
    │ Multinomial      │ n, p₁..pₖ    │ npᵢ       │ npᵢ(1-pᵢ) │ Softmax output    │
    │ Poisson          │ λ             │ λ         │ λ          │ Count data        │
    │ Geometric        │ p             │ 1/p       │ (1-p)/p²   │ Time to first hit │
    │ Neg. Binomial    │ r, p          │ r/p       │ r(1-p)/p²  │ Overdispersed ct. │
    │ Hypergeometric   │ N, K, n       │ nK/N      │ complex    │ Without-replace.  │
    │ Uniform          │ a, b          │ (a+b)/2   │ (b-a)²/12  │ Random init       │
    │ Gaussian         │ μ, σ²         │ μ         │ σ²         │ Universal default │
    │ Exponential      │ λ             │ 1/λ       │ 1/λ²       │ Time-to-event     │
    │ Gamma            │ α, β          │ α/β       │ α/β²       │ Conjugate Poisson │
    │ Beta             │ α, β          │ α/(α+β)   │ complex    │ Prior for probs   │
    │ Dirichlet        │ α₁...αₖ       │ αᵢ/Σα     │ complex    │ Prior for cats    │
    │ Chi-squared      │ k             │ k         │ 2k         │ Variance tests    │
    │ Student's t      │ ν             │ 0         │ ν/(ν-2)    │ Small-sample inf. │
    │ F                │ d₁, d₂        │ d₂/(d₂-2) │ complex    │ ANOVA / F-test    │
    │ Laplace          │ μ, b          │ μ         │ 2b²        │ L1 / Lasso prior  │
    │ Cauchy           │ x₀, γ         │ UNDEF.    │ UNDEF.     │ Heavy-tailed noise│
    └──────────────────┴───────────────┴───────────┴────────────┴───────────────────┘
...
### Modes of Convergence — Complete Hierarchy

Let Xₙ be a sequence of random variables and X a target random variable.

ALMOST SURE CONVERGENCE  (→ᵃ·ˢ·):
    ℙ(lim_{n→∞} Xₙ = X) = 1
    "The sequence converges on every sample path, except possibly a
    probability-zero set."  — Strongest mode.
    Example: Strong LLN — X̄ₙ →ᵃ·ˢ· μ

CONVERGENCE IN PROBABILITY  (→ᵖ):
    ∀ε > 0: ℙ(|Xₙ − X| > ε) → 0 as n → ∞
    "Eventually, large deviations become arbitrarily rare."
    Example: Weak LLN — X̄ₙ →ᵖ μ

CONVERGENCE IN MEAN (L² / Mean-Square Convergence)  (→^{L²}):
    E[|Xₙ − X|²] → 0 as n → ∞
    "The expected squared distance to the limit vanishes."
    Equivalent to: Var(Xₙ) → 0 AND E[Xₙ] → E[X]
    Example: Sample mean X̄ₙ →^{L²} μ  (since Var(X̄ₙ) = σ²/n → 0)

CONVERGENCE IN DISTRIBUTION  (→ᵈ):
    Fₙ(x) → F(x)  at all continuity points x of F
    "The CDFs converge pointwise." — Weakest mode.
    Example: CLT — √n(X̄ₙ−μ)/σ →ᵈ N(0,1)

    ┌────────────────────────────────────────────────────────────┐
    │  Hierarchy:  a.s.  →  in probability  →  in distribution  │
    │              L²    →  in probability                       │
    │  None of the reverse implications hold in general.         │
    │                                                            │
    │  Slutsky's theorem: if Xₙ →ᵈ X and Yₙ →ᵖ c (const):      │
    │    Xₙ + Yₙ →ᵈ X + c    and    Xₙ · Yₙ →ᵈ c · X           │
    └────────────────────────────────────────────────────────────┘
...
A COPULA separates the marginal distributions of X and Y from their
dependence structure.  Sklar's theorem (1959) is the foundation:

    SKLAR'S THEOREM: For any joint CDF F(x,y) with marginals F_X and F_Y,
    there exists a copula C: [0,1]² → [0,1] such that:

        F(x, y) = C(F_X(x), F_Y(y))

    Conversely, given any marginals F_X, F_Y and any copula C, the
    right-hand side is a valid joint CDF.

Intuition: transform X → U = F_X(X) and Y → V = F_Y(Y).
Then U, V ∈ [0,1] are uniform marginals, and C(u,v) = P(U≤u, V≤v)
captures the dependence structure independently of the marginals.

COMMON COPULAS:
    Independence copula:  C(u,v) = u·v         [X ⊥ Y]
    Gaussian copula:      C(u,v) = Φ₂(Φ⁻¹(u), Φ⁻¹(v); ρ)
                          (Φ₂ = bivariate Normal CDF, ρ = correlation)
    t-copula:             like Gaussian but with heavier joint tails
    Clayton copula:       stronger lower-tail dependence
    Gumbel copula:        stronger upper-tail dependence

    ┌──────────────────────────────────────────────────────────┐
    │  Why copulas matter:                                      │
    │  · Two variables can have any marginals AND any           │
    │    dependence structure — copulas disentangle them.       │
    │  · The "correlation" of two Gaussians is NOT the only     │
    │    way to describe their relationship.                    │
    │  · Tail dependence (joint extremes) is copula-specific:  │
    │    Gaussian copula: zero tail dependence → underestimates │
    │    joint crashes (infamous in 2008 CDO pricing models)    │
    └──────────────────────────────────────────────────────────┘

ML relevance: normalising flows learn copula-like transformations;
multi-output Gaussian processes use structured covariance functions
that effectively define a copula on the output space.
...
If Y = g(X) for a monotone differentiable function g, the PDF of Y is:

    f_Y(y) = f_X(g⁻¹(y)) · |d/dy g⁻¹(y)|    (Jacobian formula)

    The |Jacobian| corrects for stretching/compression of the
    probability mass by the transformation g.

Example — if X ~ Uniform(0,1) and Y = −log(X):
    g⁻¹(y) = e⁻ʸ,   |d/dy e⁻ʸ| = e⁻ʸ
    f_Y(y) = 1 · e⁻ʸ = e⁻ʸ   →   Y ~ Exponential(1)
    This is the INVERSE CDF (probability integral transform) method!

PROBABILITY INTEGRAL TRANSFORM:
    If X has continuous CDF F_X, then U = F_X(X) ~ Uniform(0,1).
    Conversely, X = F_X⁻¹(U) transforms Uniform into any distribution.
    → Algorithm for generating any RV from uniform samples.

MULTIVARIATE CHANGE OF VARIABLES:
    For Y = g(X) with X, Y ∈ ℝⁿ and g bijective:

        f_Y(y) = f_X(g⁻¹(y)) · |det J_{g⁻¹}(y)|

    where J is the Jacobian matrix of partial derivatives.

    Normalising flows use learnable bijections g_θ to transform a
    simple base distribution (e.g. Gaussian) into complex data distributions,
    with the Jacobian determinant tracking the density change exactly.


### Sums of Random Variables

If X and Y are independent, the density of Z = X + Y is the CONVOLUTION:

    f_Z(z) = ∫ f_X(z − y) f_Y(y) dy = (f_X * f_Y)(z)

Equivalently in terms of characteristic functions (Fourier domain):
    φ_{X+Y}(t) = φ_X(t) · φ_Y(t)   (convolution → multiplication in freq. domain)

KEY RESULTS (sums of independent RVs):

    X ~ N(μ₁,σ₁²), Y ~ N(μ₂,σ₂²):  X+Y ~ N(μ₁+μ₂, σ₁²+σ₂²)
        (Gaussians are closed under addition — the "reproductive property")

    X ~ Poisson(λ₁), Y ~ Poisson(λ₂):  X+Y ~ Poisson(λ₁+λ₂)

    X ~ Binomial(n₁,p), Y ~ Binomial(n₂,p):  X+Y ~ Binomial(n₁+n₂, p)

    X ~ Gamma(α₁,β), Y ~ Gamma(α₂,β):  X+Y ~ Gamma(α₁+α₂, β)

    Sum of k iid Exponential(λ) ~ Gamma(k, λ)

    X₁,...,Xₙ ~ iid N(0,1):  Σᵢ Xᵢ² ~ χ²(n)

    ┌──────────────────────────────────────────────────────────┐
    │  CLT is the deep result about sums: regardless of the    │
    │  individual distribution, scaled sums → Gaussian.        │
    │  "The Gaussian is the attractor of convolutions."        │
    └──────────────────────────────────────────────────────────┘


### Order Statistics

Given X₁,...,Xₙ iid from f(x), sort them:  X₍₁₎ ≤ X₍₂₎ ≤ ··· ≤ X₍ₙ₎

    X₍ₖ₎ = k-th ORDER STATISTIC  (the k-th smallest value)

PDF of the k-th order statistic:

    f_{X₍ₖ₎}(x) = n! / [(k−1)!(n−k)!] · [F(x)]^{k−1} · [1−F(x)]^{n−k} · f(x)

    Intuition: the factor counts ways to choose which k−1 observations
    fall below x, which n−k fall above, and one lands at x.

EXTREMES:
    Minimum X₍₁₎:  F_{X₍₁₎}(x) = 1 − [1−F(x)]ⁿ   (all n must exceed x)
    Maximum X₍ₙ₎:  F_{X₍ₙ₎}(x) = [F(x)]ⁿ           (all n must be ≤ x)

    For Uniform(0,1):
        E[X₍ₖ₎] = k/(n+1)  — evenly spaced in expectation.
        This explains why probability integral transform outputs are
        ordered uniformly when sorted.

ML relevance:
    · Maximum (X₍ₙ₎): worst-case bound, distribution of test error peaks.
    · Quantile estimation (median = X₍⌈n/2⌉₎): robust statistics.
    · Extreme value theory: distribution of X₍ₙ₎ → Gumbel/Fréchet/Weibull.
    · Top-k selection in neural networks (routing, attention sparsity).
    · Rank-based losses (listwise ranking in learning-to-rank).


### PART 12 — CONCENTRATION INEQUALITIES

Concentration inequalities bound the probability that a random variable
deviates far from its expectation — the mathematical foundation of
generalisation theory and PAC learning.
...
Proof: E[X] = ∫₀^∞ x f(x)dx ≥ ∫_t^∞ x f(x)dx ≥ t·P(X ≥ t)   ∎

    Diagram:
    P(X ≥ t) ≤ E[X]/t → if E[X]=1 and t=10, at most 10% of mass is ≥ 10.

This is the weakest useful inequality — it only requires finite mean.
All other concentration inequalities build on Markov's.


### Chebyshev's Inequality

For any random variable X with mean μ and variance σ², for k > 0:

    P(|X − μ| ≥ k) ≤ σ²/k²    (equivalently: P(|X−μ| ≥ kσ) ≤ 1/k²)

Proof: Apply Markov to Y = (X−μ)².

    At k=2σ:  P(|X−μ| ≥ 2σ) ≤ ¼  → at most 25% of mass is 2σ away.
    The Normal achieves 4.5% — Chebyshev is tight for worst-case distributions.
...
For independent RVs X₁,...,Xₙ with Xᵢ ∈ [0,1], S = ΣXᵢ, μ = E[S]:

    Upper tail:  P(S ≥ (1+δ)μ) ≤ exp(−μδ²/3)   for 0 < δ ≤ 1

    Lower tail:  P(S ≤ (1−δ)μ) ≤ exp(−μδ²/2)   for 0 < δ < 1

Derivation: Apply Markov to the MGF M_{S−μ}(t) and optimise over t.

    ┌──────────────────────────────────────────────────────────┐
    │  Chernoff vs Chebyshev:                                   │
    │  Chebyshev: polynomial decay O(1/k²)                     │
    │  Chernoff:  exponential decay exp(−ck²) — FAR tighter    │
    │  Works for sums of bounded independent variables.         │
    └──────────────────────────────────────────────────────────┘

ML application: bounding the number of misclassified examples in a
random classifier; analysis of random feature methods.


### Hoeffding's Inequality

For independent RVs X₁,...,Xₙ with Xᵢ ∈ [aᵢ, bᵢ], and X̄ₙ = (1/n)ΣXᵢ:

    P(X̄ₙ − E[X̄ₙ] ≥ t) ≤ exp(−2n²t² / Σᵢ(bᵢ−aᵢ)²)

For iid Xᵢ ∈ [a, b]:

    P(|X̄ₙ − μ| ≥ t) ≤ 2 exp(−2nt²/(b−a)²)

    Sample size to guarantee |X̄ₙ−μ| < ε with probability 1−δ:

        n ≥ (b−a)² log(2/δ) / (2ε²)

    ┌──────────────────────────────────────────────────────────┐
    │  Hoeffding is the workhorse of PAC learning theory:      │
    │  With n training examples, the gap between training and  │
    │  test error is bounded with high probability.            │
    │  Formalises "more data → better generalisation."         │
    └──────────────────────────────────────────────────────────┘

ML application: generalisation bounds, confidence intervals for
empirical risk minimisation, bandit algorithms (UCB uses Hoeffding).
...
For a CONVEX function φ and random variable X:
    ...
    φ(E[X]) ≤ E[φ(X)]    ("function of the mean ≤ mean of the function")

For a CONCAVE function φ: φ(E[X]) ≥ E[φ(X)]

    Convex functions: x², eˣ, |x|, xlog x, max(0,x)
    Concave functions: log x, √x, −x², entropy H(p)

Proof: for convex φ, the tangent line at E[X] lies below φ everywhere.

KEY APPLICATIONS in ML:

    KL non-negativity:  KL(p‖q) = E_p[log(p/q)] ≥ log(E_p[p/q]) = 0
        (Jensen with convex −log, applied to p/q)

    Log-sum inequality:  log(E[X]) ≤ E[log X] ... wait: concave log!
        log is concave → E[log X] ≤ log(E[X])

    ELBO derivation in VAEs:
        log p(x) = log E_{z~q}[p(x,z)/q(z)]
                 ≥ E_{z~q}[log p(x,z)/q(z)]  = ELBO    (Jensen, log concave)

    Information inequality:
        H(X) = E[−log p(X)] ≤ log|X|   (uniform maximises entropy — Jensen)


### PART 13 — STOCHASTIC PROCESSES

### Markov Chains

A MARKOV CHAIN is a sequence of random variables X₀, X₁, X₂, ... 
satisfying the MARKOV PROPERTY:

    P(Xₙ₊₁ = j | X₀,...,Xₙ) = P(Xₙ₊₁ = j | Xₙ)

"The future depends on the present, not the past." — the chain has no memory.

TRANSITION MATRIX P: for a discrete chain on states {1,...,k}:

    Pᵢⱼ = P(Xₙ₊₁ = j | Xₙ = i)    (each row sums to 1 — stochastic matrix)

    After n steps:  P(Xₙ = j | X₀ = i) = [Pⁿ]ᵢⱼ
...
STATIONARY DISTRIBUTION π:
    ...
    π is the left eigenvector of P for eigenvalue 1.
    Under ergodicity (irreducible + aperiodic):
        πⱼ = lim_{n→∞} Pⁿᵢⱼ   for all starting states i
        πⱼ = 1 / (mean return time to state j)
...
DETAILED BALANCE (reversibility):
    ...
    MCMC (Metropolis-Hastings) is designed to satisfy detailed balance.


### Stationary Distributions and Mixing

ERGODIC THEOREM: For an ergodic (irreducible, aperiodic) chain:

    (1/n) Σₜ₌₀ⁿ f(Xₜ)  →  Σᵢ πᵢ f(i) = E_π[f]   almost surely

    Time averages = space averages under the stationary distribution.
    This is why MCMC works: time-averaged samples converge to E_π[f(X)].

MIXING TIME: Number of steps needed to get ε-close to stationarity:
    τ_mix(ε) = min{t: max_i ‖P^t(i,·) − π‖_TV ≤ ε}

SPECTRAL GAP: For reversible chains:
    τ_mix ≈ 1/(1 − λ₂)   where λ₂ is the second-largest eigenvalue of P.
    Larger gap → faster mixing → fewer MCMC samples needed.

ML relevance: PageRank is the stationary distribution of a random walk
on the web graph; reinforcement learning uses the stationary distribution
of the agent's policy; diffusion models use reverse Markov chains.


### Poisson Processes

A POISSON PROCESS with rate λ is a counting process N(t) where:

    1. N(0) = 0
    2. Independent increments: counts in disjoint intervals are independent.
    3. Stationary increments: N(t+s) − N(t) ~ Poisson(λs) for all t, s.

    N(t) = number of events in [0, t]   ~   Poisson(λt)

INTER-ARRIVAL TIMES: The time between consecutive events:
    T₁, T₂, ... ~ iid Exponential(λ)

    Exponential is the ONLY memoryless continuous distribution —
    past waiting time gives no information about remaining wait.

SUPERPOSITION: If N₁(t) ~ Poisson(λ₁) and N₂(t) ~ Poisson(λ₂) independent:
    N₁(t) + N₂(t) ~ Poisson((λ₁+λ₂)t)   (Poisson processes add)

THINNING: Retain each event independently with probability p:
    Retained process ~ Poisson(pλt)

ML relevance: event sequences in NLP (word arrivals), recommendation systems
(click arrivals), queueing theory (request arrivals to servers), survival
analysis, and temporal point processes in generative modelling.
...
A GAUSSIAN PROCESS (GP) is a distribution over FUNCTIONS:
    ...
    [f(x₁),...,f(xₙ)] ~ N(μ, K)   where μᵢ = m(xᵢ),  Kᵢⱼ = k(xᵢ, xⱼ)

    m(x) — mean function (often zero for regression priors)
    k(x, x') — COVARIANCE KERNEL (encodes assumptions about smoothness)

COMMON KERNELS:
    Squared Exponential (RBF): k(x,x') = σ² exp(−‖x−x'‖²/2l²)
        Infinitely differentiable — very smooth functions.  l = length scale.
    Matérn:  k(x,x') ∝ (√(2ν/l)‖x−x'‖)^ν K_ν(...)
        Controlled smoothness; Matérn-1/2 = Ornstein-Uhlenbeck (non-diff.)
    Periodic: k(x,x') = σ² exp(−2sin²(π|x−x'|/p)/l²)
        Periodic functions with period p.

GP REGRESSION (Gaussian Process Regression, GPR):
    Prior:       f ~ GP(0, k)
    Likelihood:  y = f(X) + ε,   ε ~ N(0, σ²I)
    Posterior:   f* | X, y, X* ~ GP(μ*, K*)

    μ* = K(X*,X)[K(X,X) + σ²I]⁻¹ y        (posterior mean)
    K*  = K(X*,X*) − K(X*,X)[K(X,X)+σ²I]⁻¹K(X,X*)  (posterior covariance)

    ┌──────────────────────────────────────────────────────────┐
    │  GP as a Bayesian neural network (infinite width limit): │
    │  A neural network with one hidden layer and Gaussian     │
    │  weights converges to a GP as width → ∞ (Neal 1996).    │
    │  Modern "Neural Tangent Kernel" theory extends this to   │
    │  deep networks trained with gradient descent.            │
    └──────────────────────────────────────────────────────────┘

ML uses: probabilistic regression with uncertainty, hyperparameter
optimisation (Bayesian optimisation), spatial interpolation (kriging).


### Brownian Motion (Wiener Process)

A STANDARD BROWNIAN MOTION W(t) is a continuous-time stochastic process:

    1. W(0) = 0
    2. Independent increments: W(t)−W(s) ⊥ W(s)−W(r)  for r < s < t
...
Properties:
    ...
    NOT differentiable anywhere a.s. — infinite variation in every interval
    "Fractal" paths with Hausdorff dimension = 3/2

GEOMETRIC BROWNIAN MOTION (GBM) — the Black-Scholes model:
    dS = μS dt + σS dW
    Solution: S(t) = S(0) exp((μ − σ²/2)t + σW(t))

STOCHASTIC DIFFERENTIAL EQUATIONS (SDEs):
    dX = f(X,t) dt + g(X,t) dW    (Itô form)

    Itô's lemma (chain rule for SDEs): if Y = h(X,t):
    dY = (∂h/∂t + f·∂h/∂X + ½g²·∂²h/∂X²) dt + g·∂h/∂X dW

ML relevance: diffusion models (DDPM, Score-based models) define a
forward SDE that gradually adds noise and a reverse SDE that denoises;
Langevin dynamics uses Brownian motion to escape local minima (SGLD).


### Martingales

A sequence {Xₙ, n≥0} is a MARTINGALE with respect to filtration {ℱₙ} if:

    1. Xₙ is ℱₙ-measurable (Xₙ depends only on info up to time n)
    2. E[|Xₙ|] < ∞
    3. E[Xₙ₊₁ | ℱₙ] = Xₙ    (conditional expectation = current value)

    "A fair game: expected future value equals current value."

TYPES:
    Super-martingale:   E[Xₙ₊₁ | ℱₙ] ≤ Xₙ  (expected to decrease)
    Sub-martingale:     E[Xₙ₊₁ | ℱₙ] ≥ Xₙ  (expected to increase)

EXAMPLES:
    · Random walk Sₙ = X₁+···+Xₙ with E[Xᵢ]=0:  martingale
    · Sₙ² − nσ²:  martingale (variance grows predictably)
    · Likelihood ratio Lₙ = p(X₁,...,Xₙ|H₁)/p(X₁,...,Xₙ|H₀):  martingale under H₀

KEY THEOREMS:
    OPTIONAL STOPPING (Doob):  Under regularity conditions,
        E[X_T] = E[X₀]  for any stopping time T.
        A martingale gambler cannot gain in expectation by timing the stop.

    DOOB'S MAXIMAL INEQUALITY:
        P(max_{0≤k≤n} Xₖ ≥ λ) ≤ E[Xₙ⁺]/λ   (for non-negative sub-martingales)

    MARTINGALE CONVERGENCE (Doob):  If supₙ E[|Xₙ|] < ∞, then Xₙ → X∞ a.s.

ML relevance: SGD iterates can be analysed as noisy martingale-like
sequences; Q-learning convergence proofs use stochastic approximation
theory built on martingale methods; the optional stopping theorem
underlies sequential hypothesis testing (SPRT) and online learning.
...
''',
    ...
    # ── 10 ────────────────────────────────────────────────────────────────────
    "11 · Missing Distributions — Gamma, t, F, Laplace, Cauchy & Concentration": {
        "description": (
            "Plot and compare Gamma, Chi-squared, Student's t, F, Laplace and "
            "Cauchy distributions. Numerically verify all five concentration "
            "inequalities (Markov, Chebyshev, Chernoff, Hoeffding, Jensen) and "
...
print("=" * 65)
print("  MISSING DISTRIBUTIONS & CONCENTRATION INEQUALITIES")
print("=" * 65)
print()

# ── PART 1: Distribution properties ──────────────────────────────────────
print("  PART 1 — DISTRIBUTION PROPERTIES")
print()

dists = [
    ("Gamma(2,1)",     stats.gamma(a=2, scale=1),    "steelblue"),
    ("Gamma(5,1)",     stats.gamma(a=5, scale=1),    "royalblue"),
    ("Chi2(3)",        stats.chi2(df=3),              "tomato"),
    ("Chi2(10)",       stats.chi2(df=10),             "firebrick"),
    ("t(3)",           stats.t(df=3),                 "seagreen"),
    ("t(30)",          stats.t(df=30),                "mediumseagreen"),
    ("F(5,10)",        stats.f(dfn=5, dfd=10),        "purple"),
    ("Laplace(0,1)",   stats.laplace(loc=0, scale=1), "orange"),
    ("Cauchy(0,1)",    stats.cauchy(loc=0, scale=1),  "crimson"),
]

print(f"  {'Distribution':18s} | {'Mean':>8} | {'Std':>8} | {'Skew':>8} | {'Ex.Kurt':>8}")
print(f"  {'─'*62}")
for name, d, _ in dists:
    try:
        mean = d.mean()
        std  = d.std()
        skew = d.stats("s")
        kurt = d.stats("k")
        mean_s = f"{mean:8.3f}" if abs(mean) < 1e5 else "    UNDEF"
        std_s  = f"{std:8.3f}"  if abs(std)  < 1e5 else "    UNDEF"
        skew_s = f"{float(skew):8.3f}" if np.isfinite(float(skew)) else "    UNDEF"
        kurt_s = f"{float(kurt):8.3f}" if np.isfinite(float(kurt)) else "    UNDEF"
        print(f"  {name:18s} | {mean_s} | {std_s} | {skew_s} | {kurt_s}")
    except Exception:
        print(f"  {name:18s} | {'UNDEF':>8} | {'UNDEF':>8} | {'UNDEF':>8} | {'UNDEF':>8}")

print()
print("  Cauchy: undefined mean and variance — law of large numbers FAILS.")
print("  t(3):   infinite variance — heavier tails than Normal.")
print()

# ── PART 2: Concentration Inequalities ───────────────────────────────────
print("  PART 2 — CONCENTRATION INEQUALITIES (empirical verification)")
print()

n_samples = 100_000
X = np.random.exponential(2, n_samples)   # Exp(0.5), mean=2, var=4
true_mean = 2.0
true_var  = 4.0

print(f"  Distribution: Exponential(rate=0.5),  E[X]={true_mean}, Var(X)={true_var}")
print()
print(f"  P(X ≥ t) for various thresholds:")
print()
print(f"  {'t':>5} | {'Empirical':>10} | {'Markov':>10} | {'Chebyshev':>12} | {'Notes'}")
print(f"  {'─'*65}")

for t in [4, 6, 8, 12, 20]:
    emp  = (X >= t).mean()
    mark = min(true_mean / t, 1.0)
    cheb = min(true_var / (t - true_mean)**2, 1.0) if t > true_mean else 1.0
    note = "tight" if abs(emp - mark) < 0.05 * mark else ""
    print(f"  {t:5d} | {emp:10.6f} | {mark:10.6f} | {cheb:12.6f} | {note}")

print()
print("  Markov is tighter than Chebyshev at large t for Exponential.")
print()

# Hoeffding's inequality for bounded variables
print("  Hoeffding: X₁,...,Xₙ ∈ [0,1], testing if mean > E[X̄] + t")
Xb = np.random.uniform(0, 1, n_samples)
true_mean_b = 0.5
print(f"  True mean = {true_mean_b}")
print()
print(f"  {'n':>5} | {'t':>6} | {'Empirical P':>12} | {'Hoeffding bound':>16}")
print(f"  {'─'*50}")
for n_h, t_h in [(10, 0.2), (30, 0.15), (100, 0.1), (500, 0.05)]:
    Xmeans = np.array([np.random.uniform(0,1,n_h).mean() for _ in range(20000)])
    emp_h  = (Xmeans >= true_mean_b + t_h).mean()
    hoeff  = np.exp(-2*n_h*t_h**2)   # Hoeffding for Uniform in [0,1]
    print(f"  {n_h:5d} | {t_h:6.3f} | {emp_h:12.6f} | {hoeff:16.6f}")

print()

# Jensen's inequality demonstration
print("  Jensen: E[log(X)] ≤ log(E[X])  (log is concave)")
X_pos = np.random.exponential(1, n_samples)
print(f"  E[log(X)] = {np.log(X_pos).mean():.5f}   (empirical)")
print(f"  log(E[X]) = {np.log(X_pos.mean()):.5f}   (log of mean)")
print(f"  Jensen gap = {np.log(X_pos.mean()) - np.log(X_pos).mean():.5f}  ≥ 0  ✓")
print()

# ── Plots ─────────────────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 3, figsize=(17, 5))
fig.suptitle("Missing Distributions & Concentration Inequalities",
             fontsize=12, fontweight="bold")

# Plot 1: Gamma and Chi-squared
x1 = np.linspace(0, 20, 300)
for name, d, col in dists[:4]:
    axes[0].plot(x1, d.pdf(x1), lw=2, color=col, label=name)
axes[0].set_title("Gamma and Chi-squared\\n(related via Gamma(k/2,1/2)=Chi2(k))")
axes[0].set_xlim(0, 20); axes[0].set_ylim(0, 0.5)
axes[0].set_xlabel("x"); axes[0].legend(fontsize=8); axes[0].grid(alpha=0.3)

# Plot 2: t, Laplace, Cauchy vs Normal
x2 = np.linspace(-6, 6, 400)
axes[1].plot(x2, stats.norm.pdf(x2), "k--", lw=2, label="Normal(0,1)")
for name, d, col in dists[4:8]:
    axes[1].plot(x2, d.pdf(x2), lw=2, color=col, label=name)
axes[1].plot(x2, stats.cauchy.pdf(x2), lw=2, color="crimson",
             linestyle=":", label="Cauchy(0,1)")
axes[1].set_title("Heavy-tailed distributions\\nvs Normal (dashed)")
axes[1].set_xlim(-5, 5); axes[1].set_ylim(0, 0.5)
axes[1].set_xlabel("x"); axes[1].legend(fontsize=8); axes[1].grid(alpha=0.3)

# Plot 3: Concentration inequalities comparison
ts = np.linspace(2.1, 15, 200)
emp_vals = [(X >= t).mean() for t in ts]
mark_vals = np.minimum(true_mean / ts, 1.0)
cheb_vals = np.minimum(true_var / (ts - true_mean)**2, 1.0)
hoeff_vals = np.exp(-2 * (ts - true_mean)**2 / (4**2))  # rough Hoeffding

axes[2].semilogy(ts, emp_vals,  "k-",   lw=2, label="Empirical P(X≥t)")
axes[2].semilogy(ts, mark_vals, "tomato", lw=2, linestyle="--", label="Markov")
axes[2].semilogy(ts, cheb_vals, "steelblue", lw=2, linestyle="-.", label="Chebyshev")
axes[2].semilogy(ts, hoeff_vals,"seagreen", lw=2, linestyle=":", label="Hoeffding-type")
axes[2].set_xlabel("threshold t"); axes[2].set_ylabel("Probability (log scale)")
axes[2].set_title("Concentration Inequalities\\n(Exponential(0.5) distribution)")
axes[2].legend(fontsize=8); axes[2].grid(alpha=0.3)
axes[2].set_xlim(2, 15)

plt.tight_layout()
plt.savefig(OUTPUT_DIR / "missing_dists_concentration.png", dpi=120)
print("  Plot saved → missing_dists_concentration.png")
print()
print("  KEY TAKEAWAYS:")
print("  Gamma(α,β): flexible positive-support dist; Gamma(1,β)=Exp(β).")
print("  Chi2(k): sum of k squared N(0,1); basis of variance tests.")
print("  t(ν): heavier tails than Normal; → Normal as ν → ∞.")
print("  Cauchy: no finite moments; LLN fails; PHI(t)=exp(-|t|).")
print("  Markov is the weakest but only needs E[X]<∞.")
print("  Hoeffding gives exponential decay — basis of PAC-learning bounds.")
''',
    },

    # ── 11 ────────────────────────────────────────────────────────────────────
    "12 · Transformations, Sums & Order Statistics": {
        "description": (
            "Demonstrate the change-of-variables formula (inverse CDF method). "
            "Verify sum-of-distributions theorems (convolution). "
            "Compute and visualise order statistics for Uniform(0,1) and "
            "compare empirical distributions against analytic PDFs."
        ),
        "language": "python",
        "code": '''
import numpy as np
...
print("=" * 65)
print("  TRANSFORMATIONS, SUMS & ORDER STATISTICS")
print("=" * 65)
print()

# ── PART 1: Change of variables / inverse CDF ─────────────────────────────
print("  PART 1 — CHANGE OF VARIABLES: INVERSE CDF METHOD")
print()
print("  Theorem: if U ~ Uniform(0,1) then X = F_X⁻¹(U) ~ F_X")
print()

# Generate Exponential(λ) from Uniform via F⁻¹(u) = -log(1-u)/λ
lam = 1.5
U   = np.random.uniform(0, 1, 100_000)
X_inv_cdf = -np.log(1 - U) / lam   # exact transform

X_direct  = np.random.exponential(1/lam, 100_000)  # reference

ks_stat, ks_p = stats.kstest(X_inv_cdf, lambda x: stats.expon.cdf(x, scale=1/lam))

print(f"  Generating Exponential(λ={lam}) from Uniform via F⁻¹(U) = -log(1-U)/λ")
print(f"  Inverse CDF sample:  mean={X_inv_cdf.mean():.5f}, std={X_inv_cdf.std():.5f}")
print(f"  True distribution:   mean={1/lam:.5f}, std={1/lam:.5f}")
print(f"  KS test vs Exp({lam}): stat={ks_stat:.5f}, p={ks_p:.3f} (large p → correct distribution)")
print()

# Probability integral transform: F_X(X) ~ Uniform(0,1)
X_chi2 = np.random.chisquare(5, 100_000)
U_transformed = stats.chi2.cdf(X_chi2, df=5)
ks2, p2 = stats.kstest(U_transformed, "uniform")
print(f"  Probability integral transform: F_Chi2(X) where X ~ Chi2(5)")
print(f"  KS test vs Uniform(0,1): stat={ks2:.5f}, p={p2:.3f}  ✓")
print()

# ── PART 2: Sums of random variables ─────────────────────────────────────
print("  PART 2 — SUMS OF RANDOM VARIABLES (convolution)")
print()
...
exp_samples  = np.random.exponential(1, (N_SIMS, n_sum))
sum_exp      = exp_samples.sum(axis=1)   # should be Gamma(n_sum, 1)
gamma_theory = stats.gamma(a=n_sum, scale=1)
ks3, p3      = stats.kstest(sum_exp, lambda x: gamma_theory.cdf(x))
print(f"  Sum of {n_sum} iid Exp(1) → should be Gamma({n_sum},1):")
print(f"  Sample mean = {sum_exp.mean():.4f}  (theory: {n_sum:.1f})")
print(f"  Sample std  = {sum_exp.std():.4f}  (theory: {np.sqrt(n_sum):.4f})")
print(f"  KS test vs Gamma({n_sum},1): p = {p3:.4f}  ✓")
print()

# Sum of Normals → Normal
mu1, s1 = 2.0, 1.5
mu2, s2 = 1.0, 2.0
X1 = np.random.normal(mu1, s1, N_SIMS)
X2 = np.random.normal(mu2, s2, N_SIMS)
Z  = X1 + X2
ks4, p4 = stats.kstest(Z, lambda x: stats.norm.cdf(x, mu1+mu2, np.sqrt(s1**2+s2**2)))
print(f"  N({mu1},{s1}²) + N({mu2},{s2}²) → should be N({mu1+mu2},{s1**2+s2**2:.1f}):")
print(f"  Sample mean = {Z.mean():.4f}  (theory: {mu1+mu2:.4f})")
print(f"  Sample std  = {Z.std():.4f}  (theory: {np.sqrt(s1**2+s2**2):.4f})")
print(f"  KS test vs N({mu1+mu2},{s1**2+s2**2:.1f}): p = {p4:.4f}  ✓")
print()

# Sum of k squared Normals → Chi-squared(k)
k_chi = 6
Z_sq = np.random.normal(0, 1, (N_SIMS, k_chi))**2
chi2_sum = Z_sq.sum(axis=1)
ks5, p5  = stats.kstest(chi2_sum, lambda x: stats.chi2.cdf(x, df=k_chi))
print(f"  Sum of {k_chi} squared N(0,1) → should be Chi2({k_chi}):")
print(f"  Sample mean = {chi2_sum.mean():.4f}  (theory: {k_chi:.1f})")
print(f"  KS test vs Chi2({k_chi}): p = {p5:.4f}  ✓")
print()

# ── PART 3: Order Statistics ──────────────────────────────────────────────
print("  PART 3 — ORDER STATISTICS")
print()

n_ord = 10
N_ORD = 100_000

all_samples = np.random.uniform(0, 1, (N_ORD, n_ord))
all_sorted  = np.sort(all_samples, axis=1)

print(f"  n={n_ord} iid Uniform(0,1) samples. Theory: E[X_(k)] = k/(n+1)")
print()
print(f"  {'k':>4} | {'E[X_(k)] emp':>14} | {'E[X_(k)] theory':>16} | "
      f"{'Std emp':>10} | {'Std theory':>12}")
print(f"  {'─'*65}")
for k in range(1, n_ord+1):
    emp_mean  = all_sorted[:, k-1].mean()
    emp_std   = all_sorted[:, k-1].std()
    th_mean   = k / (n_ord + 1)
    # Var of k-th order stat of Uniform(0,1):
    th_var    = k * (n_ord-k+1) / ((n_ord+1)**2 * (n_ord+2))
    th_std    = np.sqrt(th_var)
    print(f"  {k:4d} | {emp_mean:14.5f} | {th_mean:16.5f} | "
          f"{emp_std:10.5f} | {th_std:12.5f}")

print()

# ── Plots ──────────────────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 3, figsize=(17, 5))
fig.suptitle("Transformations, Sums & Order Statistics",
             fontsize=12, fontweight="bold")

# Plot 1: Inverse CDF transform
x_exp = np.linspace(0, 5, 300)
axes[0].hist(X_inv_cdf, bins=60, density=True, color="steelblue",
             alpha=0.6, label="Inv. CDF samples")
axes[0].plot(x_exp, stats.expon.pdf(x_exp, scale=1/lam),
             "tomato", lw=2.5, label=f"True Exp({lam})")
axes[0].set_xlabel("x"); axes[0].set_title("Inverse CDF Method\\nExp from Uniform")
axes[0].legend(fontsize=9); axes[0].grid(alpha=0.3); axes[0].set_xlim(0, 5)

# Plot 2: Convolution — sum of Exponentials
x_g = np.linspace(0, 20, 300)
axes[1].hist(sum_exp, bins=60, density=True, color="seagreen",
             alpha=0.6, label=f"Sum of {n_sum} Exp(1)")
axes[1].plot(x_g, gamma_theory.pdf(x_g), "tomato", lw=2.5,
             label=f"Gamma({n_sum},1)")
axes[1].set_xlabel("x"); axes[1].set_title(f"Convolution\\nSum of {n_sum} Exp(1) = Gamma({n_sum},1)")
axes[1].legend(fontsize=9); axes[1].grid(alpha=0.3)

# Plot 3: Order statistic PDFs
x_os = np.linspace(0, 1, 300)
palette = plt.cm.viridis(np.linspace(0, 1, n_ord))
for k in [1, 3, 5, 7, 10]:
    # PDF of k-th order stat of Uniform(0,1) on [0,n]:
    # f(x) = n!/(k-1)!(n-k)! * x^(k-1) * (1-x)^(n-k)
    pdf_k = stats.beta.pdf(x_os, k, n_ord-k+1)
    col   = palette[k-1]
    axes[2].plot(x_os, pdf_k, lw=2, color=col, label=f"X_({k})")
    axes[2].hist(all_sorted[:, k-1], bins=40, density=True,
                 alpha=0.2, color=col)
axes[2].set_xlabel("x"); axes[2].set_title(f"Order Statistic PDFs (n={n_ord})\\nlines=theory, bars=empirical")
axes[2].legend(fontsize=8); axes[2].grid(alpha=0.3)

plt.tight_layout()
plt.savefig(OUTPUT_DIR / "transformations_order_stats.png", dpi=120)
print("  Plot saved → transformations_order_stats.png")
print()
print("  KEY TAKEAWAYS:")
print("  Inverse CDF: any RV from Uniform via F⁻¹(U) — the basis of sampling.")
print("  Sums: Exp → Gamma, Normal → Normal, χ² → χ² by additive closure.")
print("  Order stats: E[X_(k)] = k/(n+1) for Uniform — evenly spaced in expectation.")
print("  Beta(k, n-k+1) is the exact distribution of the k-th order statistic.")
''',
    },

    # ── 12 ────────────────────────────────────────────────────────────────────
    "13 · Stochastic Processes — Markov, Poisson & Gaussian Processes": {
        "description": (
            "Simulate and analyse a discrete Markov chain: find the stationary "
            "distribution via power iteration and verify it matches the eigenvector. "
            "Simulate a Poisson process and verify inter-arrival times are Exponential. "
            "Build a simple Gaussian Process regression with RBF kernel and show "
...
print("=" * 65)
print("  STOCHASTIC PROCESSES: MARKOV, POISSON & GAUSSIAN PROCESSES")
print("=" * 65)
print()

# ── PART 1: Discrete Markov Chain ─────────────────────────────────────────
print("  PART 1 — DISCRETE MARKOV CHAIN")
print()
...
print(f"  {'':8s}", end="")
for s in states: print(f"  {s:8s}", end="")
print()
for i, row_name in enumerate(states):
    print(f"  {row_name:8s}", end="")
    for p in P[i]: print(f"  {p:8.3f}", end="")
    print()
print()

# Stationary distribution: πP = π, solve as eigenvalue problem
eigenvalues, eigenvectors = np.linalg.eig(P.T)
idx = np.argmin(np.abs(eigenvalues - 1.0))
pi_eigen = np.abs(eigenvectors[:, idx])
pi_eigen /= pi_eigen.sum()

# Power iteration
pi_power = np.array([1.0, 0.0, 0.0])   # start in Sunny
for _ in range(1000):
    pi_power = pi_power @ P
print(f"  Stationary distribution (eigenvector): {dict(zip(states, pi_eigen.round(5)))}")
print(f"  Stationary distribution (power iter):  {dict(zip(states, pi_power.round(5)))}")
print()

# Verify: πP = π
residual = np.linalg.norm(pi_eigen @ P - pi_eigen)
print(f"  Verification ‖πP − π‖ = {residual:.2e}  ✓")
print()

# Detailed balance check
print("  Detailed balance check πᵢPᵢⱼ = πⱼPⱼᵢ:  (reversible chain?)")
for i in range(3):
    for j in range(i+1, 3):
        lhs = pi_eigen[i] * P[i, j]
        rhs = pi_eigen[j] * P[j, i]
        eq  = "≈" if abs(lhs-rhs) < 0.01 else "≠"
        print(f"    π_{states[i]}·P[{states[i]}→{states[j]}]={lhs:.4f}  {eq}  π_{states[j]}·P[{states[j]}→{states[i]}]={rhs:.4f}")
print()

# Simulate chain and verify empirical distribution
n_steps = 100_000
chain_sim = np.zeros(n_steps, dtype=int)
chain_sim[0] = 0
for t in range(1, n_steps):
    chain_sim[t] = np.random.choice(3, p=P[chain_sim[t-1]])

empirical = np.bincount(chain_sim) / n_steps
print(f"  Empirical vs stationary (n={n_steps:,}):")
for i, s in enumerate(states):
    print(f"    {s}: empirical={empirical[i]:.4f}, stationary={pi_eigen[i]:.4f}")
print()

# ── PART 2: Poisson Process ───────────────────────────────────────────────
print("  PART 2 — POISSON PROCESS")
print()

lam_pp = 3.0    # events per unit time
T_pp   = 50.0   # observe for T units

# Simulate by generating Exponential inter-arrival times
inter_arrivals = np.random.exponential(1/lam_pp, int(lam_pp * T_pp * 3))
arrival_times  = np.cumsum(inter_arrivals)
arrival_times  = arrival_times[arrival_times <= T_pp]
N_T = len(arrival_times)

print(f"  Rate λ={lam_pp}, observation interval [0,{T_pp}]")
print(f"  Observed events N({T_pp}) = {N_T}  (theory: E[N] = {lam_pp*T_pp:.0f})")
print()

# Count in sub-intervals → should be Poisson(λΔt)
delta_t = 5.0
counts  = []
t_start = 0
while t_start + delta_t <= T_pp:
    c = ((arrival_times >= t_start) & (arrival_times < t_start + delta_t)).sum()
    counts.append(c)
    t_start += delta_t
counts = np.array(counts)

print(f"  Counts in {delta_t}-unit windows (should be Poisson({lam_pp*delta_t:.0f})):")
print(f"  Mean = {counts.mean():.3f}  (theory: {lam_pp*delta_t:.1f})")
print(f"  Var  = {counts.var():.3f}  (theory: {lam_pp*delta_t:.1f} — mean=var for Poisson)")
ks_pp, p_pp = stats.kstest(counts, lambda x: stats.poisson.cdf(x, lam_pp*delta_t))
print(f"  KS test vs Poisson({lam_pp*delta_t:.0f}): p={p_pp:.3f}")
print()

# Inter-arrival times → should be Exponential
ks_ia, p_ia = stats.kstest(inter_arrivals[:200],
                            lambda x: stats.expon.cdf(x, scale=1/lam_pp))
print(f"  Inter-arrival times ~ Exp({lam_pp}):")
print(f"  Mean = {inter_arrivals[:200].mean():.4f}  (theory: {1/lam_pp:.4f})")
print(f"  KS test: p={p_ia:.3f}  ✓")
print()

# ── PART 3: Gaussian Process Regression ───────────────────────────────────
print("  PART 3 — GAUSSIAN PROCESS REGRESSION")
print()

def rbf_kernel(x1, x2, sigma2=1.0, length_scale=1.0):
    """Squared exponential (RBF) kernel."""
    d2 = (x1[:, None] - x2[None, :])**2
    return sigma2 * np.exp(-d2 / (2 * length_scale**2))

# Training data: noisy observations of sin(x)
X_train = np.array([-3.0, -1.5, 0.0, 1.5, 3.0])
y_train = np.sin(X_train) + np.random.normal(0, 0.15, len(X_train))
sigma_n2 = 0.15**2    # noise variance
...
K_ts   = rbf_kernel(X_train, X_test)
K_ss   = rbf_kernel(X_test,  X_test)

L_noisy = K_tt + sigma_n2 * np.eye(len(X_train))
alpha   = np.linalg.solve(L_noisy, y_train)
mu_post = K_ts.T @ alpha

v       = np.linalg.solve(L_noisy, K_ts)
var_post = np.diag(K_ss) - np.einsum("ij,ij->j", v, K_ts)
std_post = np.sqrt(np.maximum(var_post, 0))

print(f"  Training points: {X_train}")
print(f"  Noise std: {np.sqrt(sigma_n2):.2f}")
print()
print(f"  Posterior mean at test points (sample):")
for xi, mi, si in zip(X_test[::40], mu_post[::40], std_post[::40]):
    print(f"    x={xi:6.2f}: μ={mi:7.4f} ± {1.96*si:.4f} (95% CI)")
print()

# ── Plots ──────────────────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 3, figsize=(17, 5))
fig.suptitle("Stochastic Processes: Markov Chain, Poisson Process, GP Regression",
             fontsize=12, fontweight="bold")

# Plot 1: Markov chain trajectory + state distribution
traj = chain_sim[:300]
axes[0].step(range(300), traj, color="steelblue", lw=1, where="post")
axes[0].set_yticks([0,1,2]); axes[0].set_yticklabels(states, fontsize=9)
axes[0].set_xlabel("Step"); axes[0].set_title("Markov Chain Trajectory\\n(first 300 steps)")
axes[0].grid(alpha=0.3)
axes2 = axes[0].inset_axes([0.65, 0.6, 0.32, 0.35])
axes2.bar(range(3), pi_eigen, color=["gold","gray","steelblue"], alpha=0.8)
axes2.set_xticks(range(3)); axes2.set_xticklabels(states, fontsize=7)
axes2.set_title("π", fontsize=8); axes2.tick_params(labelsize=7)

# Plot 2: Poisson process arrival times
axes[1].eventplot(arrival_times[:min(N_T,100)], orientation="horizontal",
                  colors="steelblue", lineoffsets=0.5, linelengths=0.8)
axes[1].step(np.concatenate([[0], arrival_times, [T_pp]]),
             np.concatenate([[0], np.arange(1, N_T+1), [N_T]]),
             color="tomato", lw=1.5, where="post", alpha=0.7, label="N(t)")
axes[1].set_xlabel("Time"); axes[1].set_ylabel("N(t) / Events")
axes[1].set_title(f"Poisson Process (λ={lam_pp})\\nStep = cumulative count")
axes[1].legend(fontsize=8); axes[1].grid(alpha=0.3)

# Plot 3: GP regression
axes[2].fill_between(X_test, mu_post - 1.96*std_post, mu_post + 1.96*std_post,
                     alpha=0.2, color="steelblue", label="95% CI")
axes[2].plot(X_test, mu_post, "steelblue", lw=2, label="GP posterior mean")
axes[2].plot(X_test, np.sin(X_test), "tomato", lw=1.5, linestyle="--", label="True sin(x)")
axes[2].scatter(X_train, y_train, s=60, color="k", zorder=5, label="Training data")
axes[2].set_xlabel("x"); axes[2].set_ylabel("f(x)")
axes[2].set_title("GP Regression (RBF kernel)\\nUncertainty grows away from data")
axes[2].legend(fontsize=8); axes[2].grid(alpha=0.3)

plt.tight_layout()
plt.savefig(OUTPUT_DIR / "stochastic_processes.png", dpi=120)
print("  Plot saved → stochastic_processes.png")
print()
print("  KEY TAKEAWAYS:")
print("  Markov: πP=π; power iteration and eigenvectors both give stationary dist.")
print("  Detailed balance (πᵢPᵢⱼ = πⱼPⱼᵢ) → reversible chain (MCMC uses this).")
print("  Poisson process: N(Δt) ~ Poisson(λΔt), inter-arrivals ~ Exp(λ).")
print("  GP regression: posterior = Gaussian; uncertainty increases away from data.")
print("  GP kernel encodes smoothness assumptions; RBF = infinitely differentiable.")
''',
    },
