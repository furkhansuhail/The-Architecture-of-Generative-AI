...
DISPLAY_NAME = "03 · Activations & Loss Functions"
ICON         = "⚡"
...
def make_classification(n_samples=100, n_features=20, n_informative=2,
                        ...
                        weights=None, flip_y=0.0, **kw):
    rng = _np_impl.random.default_rng(random_state)
    if weights is None: weights = [0.5, 0.5]
    w = _np_impl.array(weights, dtype=float); w /= w.sum()
    counts = (w * n_samples).astype(int)
    counts[-1] = n_samples - counts[:-1].sum()
    y = _np_impl.concatenate([_np_impl.full(int(c), k, dtype=int) for k, c in enumerate(counts)])
    rng.shuffle(y)
    Xi  = rng.standard_normal((n_samples, n_informative))
    Xi += (2*y - 1)[:, None] * rng.uniform(0.8, 1.5, n_informative)
    nr  = n_redundant
    ...
    Xn  = rng.standard_normal((n_samples, nn2)) if nn2 > 0 else _np_impl.empty((n_samples, 0))
    parts = [a for a in [Xi, Xr, Xn] if a.shape[1] > 0]
    X = _np_impl.hstack(parts)[:, :n_features]
    if flip_y > 0:
...
def train_test_split(*arrays, test_size=0.3, random_state=None, stratify=None, **kw):
    ...
    if stratify is not None:
        classes = _np_impl.unique(_np_impl.asarray(stratify))
        ti, vi = [], []
        for c in classes:
            idx_c = _np_impl.where(_np_impl.asarray(stratify) == c)[0]
            rng.shuffle(idx_c)
            n_te_c = max(1, int(len(idx_c) * test_size))
            vi.extend(idx_c[:n_te_c]); ti.extend(idx_c[n_te_c:])
        ti = _np_impl.array(ti); vi = _np_impl.array(vi)
    else:
        n_tr = int(n * (1 - test_size)); idx = rng.permutation(n)
        ti, vi = idx[:n_tr], idx[n_tr:]
    out = []
    for a in arrays: out += [a[ti], a[vi]]
    return out

def f1_score(y_true, y_pred, zero_division=0):
    yt, yp = _np_impl.asarray(y_true), _np_impl.asarray(y_pred)
    tp = _np_impl.sum((yp == 1) & (yt == 1))
    fp = _np_impl.sum((yp == 1) & (yt == 0))
    fn = _np_impl.sum((yp == 0) & (yt == 1))
...
def average_precision_score(y_true, y_score):
    ...
    prec = tp / (tp + fp)
    rec  = tp / (yt.sum() + 1e-15)
    rec_prev = _np_impl.concatenate([[0.0], rec[:-1]])
    return float(_np_impl.sum(prec * (rec - rec_prev)))
...
def get_content():
    ...
    visual_height = 1150
    try:
        from Training_Core.visuals.activations_visual import (
            ACTIVATIONS_VISUAL_HTML,
            ACTIVATIONS_VISUAL_HEIGHT,
        )
        visual_html   = ACTIVATIONS_VISUAL_HTML.encode("utf-8", "surrogatepass").decode("utf-8", "ignore")
        visual_height = ACTIVATIONS_VISUAL_HEIGHT
